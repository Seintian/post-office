...
TEST(FRAMING, ROUND_TRIP_EMPTY_PAYLOAD) {
    int sv[2];
...
TEST(FRAMING, ROUND_TRIP_SMALL_PAYLOAD) {
    int sv[2];
    TEST_ASSERT_EQUAL_INT(0, socketpair(AF_UNIX, SOCK_STREAM, 0, sv));
    const char msg[] = "hi";
    po_header_t h;
...
TEST_GROUP_RUNNER(FRAMING) {
    RUN_TEST_CASE(FRAMING, ROUND_TRIP_EMPTY_PAYLOAD);
    RUN_TEST_CASE(FRAMING, ROUND_TRIP_SMALL_PAYLOAD);
}

// Additional tests

TEST(FRAMING, INIT_AND_GET_MAX_PAYLOAD) {
    // default
...
TEST(FRAMING, READ_REJECTS_TOTAL_SMALLER_THAN_HEADER) {
    int sv[2];
    TEST_ASSERT_EQUAL_INT(0, socketpair(AF_UNIX, SOCK_STREAM, 0, sv));
...
TEST(FRAMING, READ_REJECTS_BAD_VERSION) {
    int sv[2];
    TEST_ASSERT_EQUAL_INT(0, socketpair(AF_UNIX, SOCK_STREAM, 0, sv));
...
TEST(FRAMING, READ_REJECTS_TOO_LARGE_PAYLOAD) {
...
TEST(FRAMING, WRITE_REJECTS_TOO_LARGE_PAYLOAD) {
    // Set small max
...
TEST(FRAMING, WRITE_ZERO_COPY_TREATED_AS_ZERO_PAYLOAD) {
    int sv[2];
    TEST_ASSERT_EQUAL_INT(0, socketpair(AF_UNIX, SOCK_STREAM, 0, sv));
    po_header_t h;
...
TEST_GROUP_RUNNER(FRAMING_EXT) {
    RUN_TEST_CASE(FRAMING, INIT_AND_GET_MAX_PAYLOAD);
    RUN_TEST_CASE(FRAMING, READ_REJECTS_TOTAL_SMALLER_THAN_HEADER);
    RUN_TEST_CASE(FRAMING, READ_REJECTS_BAD_VERSION);
    RUN_TEST_CASE(FRAMING, READ_REJECTS_TOO_LARGE_PAYLOAD);
    RUN_TEST_CASE(FRAMING, WRITE_REJECTS_TOO_LARGE_PAYLOAD);
    RUN_TEST_CASE(FRAMING, WRITE_ZERO_COPY_TREATED_AS_ZERO_PAYLOAD);
}
