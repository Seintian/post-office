...
TEST(PROTOCOL, INIT_AND_BYTE_ORDER_ROUND_TRIP) {
    po_header_t h = {0};
...
TEST_GROUP_RUNNER(PROTOCOL) {
    RUN_TEST_CASE(PROTOCOL, INIT_AND_BYTE_ORDER_ROUND_TRIP);
}

TEST(PROTOCOL, MESSAGE_SIZE_COMPUTATION) {
...
TEST(PROTOCOL, LARGE_PAYLOAD_BOUNDARY) {
...
TEST_GROUP_RUNNER(PROTOCOL_EXT) {
    RUN_TEST_CASE(PROTOCOL, MESSAGE_SIZE_COMPUTATION);
    RUN_TEST_CASE(PROTOCOL, LARGE_PAYLOAD_BOUNDARY);
}
