...
static int wait_get(po_logstore_t *ls, const void *key, size_t klen, void **out, size_t *outlen,
    ...
    while (waited < timeout_ms * 1000) {
        if (po_logstore_get(ls, key, klen, out, outlen) == 0)
            return 0;
        usleep(step_us);
        waited += step_us;
    }
...
TEST(LOGSTORE, APPEND_MULTIPLE_UNIQUE) {
    ...
    for (size_t i = 0; i < 4; ++i) {
        TEST_ASSERT_EQUAL_INT(
            0, po_logstore_append(g_ls, keys[i], strlen(keys[i]), vals[i], strlen(vals[i])));
    }
    for (size_t i = 0; i < 4; ++i) {
        void *out = NULL;
        size_t outlen = 0;
...
TEST(LOGSTORE, PERSISTENCE_TRUNCATED_LAST_RECORD) {
    ...
    const char *k2 = "k_two";
    const char *v2 = "value_that_will_be_truncated";
    ...
    TEST_ASSERT_EQUAL_UINT(strlen(v1), outlen);
    free(out);
...
TEST(LOGSTORE, REBUILD_ON_OPEN_WITH_TRUNCATE) {
    ...
    void *out = NULL;
    size_t outlen = 0;
    TEST_ASSERT_EQUAL_INT(0, wait_get(g_ls, k1, strlen(k1), &out, &outlen, 300));
    free(out);
    po_logstore_close(&g_ls);
    ...
    TEST_ASSERT_TRUE(fd >= 0);
    uint32_t bogus = 0xFFFFFFFF;
    ssize_t wr = write(fd, &bogus, sizeof(bogus));
    (void)wr; // incomplete record
    close(fd);
...
TEST(LOGSTORE, TAIL_TRUNCATION_PARTIAL_VALUE_REBUILD) {
    ...
    const char *v1 = "AAAA";
    const char *k2 = "pv_k2";
    const char *v2 = "BBBBBBBB"; // we'll truncate half
    TEST_ASSERT_EQUAL_INT(0, po_logstore_append(g_ls, k1, strlen(k1), v1, strlen(v1)));
    TEST_ASSERT_EQUAL_INT(0, po_logstore_append(g_ls, k2, strlen(k2), v2, strlen(v2)));
    void *out = NULL;
    size_t outlen = 0;
    ...
    TEST_ASSERT_EQUAL_INT(0, ftruncate(fd, sz - 4));
    close(fd);
    po_logstore_cfg cfg2 = {.dir = g_dir,
                            .bucket = "idx",
                            .map_size = 1 << 20,
                            .ring_capacity = 64,
                            .batch_size = 4,
...
TEST(LOGSTORE, LOGGER_SINK_ATTACHED_WRITES_FILE) {
    ...
    fstat(open(path, O_RDONLY), &st_before); // ignore error
...
TEST(LOGSTORE, APPEND_EXCEED_MAX_KEY) {
    ...
    void *out = NULL;
    size_t outlen = 0;
    TEST_ASSERT_EQUAL_INT(0, wait_get(g_ls, key, 16, &out, &outlen, 300));
    free(out);
}
...
TEST(LOGSTORE, APPEND_EXCEED_MAX_VALUE) {
    ...
    void *out = NULL;
    size_t outlen = 0;
    TEST_ASSERT_EQUAL_INT(0, wait_get(g_ls, "k", 1, &out, &outlen, 300));
    free(out);
}

TEST(LOGSTORE, DEBUG_LOOKUP_RETURNS_OFFSET) {
    const char *k = "dblk";
    const char *v = "debug_lookup_value";
    TEST_ASSERT_EQUAL_INT(0, po_logstore_append(g_ls, k, strlen(k), v, strlen(v)));
    void *out = NULL;
    size_t outlen = 0;
    TEST_ASSERT_EQUAL_INT(0, wait_get(g_ls, k, strlen(k), &out, &outlen, 400));
    free(out);
    uint64_t off = 0;
    uint32_t len = 0;
    TEST_ASSERT_EQUAL_INT(0, po_logstore_debug_lookup(g_ls, k, strlen(k), &off, &len));
    TEST_ASSERT_EQUAL_UINT(strlen(v), len);
    // sanity: reading via get again returns same length
    TEST_ASSERT_EQUAL_INT(0, po_logstore_get(g_ls, k, strlen(k), &out, &outlen));
    TEST_ASSERT_EQUAL_UINT(len, outlen);
    free(out);
}
...
TEST(LOGSTORE, FAULT_INJECTION_FALLBACK_PATH) {
    ...
    for (int i = 0; i < 40; i += 1) {
        ...
        int ret = wait_get(g_ls, k, strlen(k), &out, &outlen, 800);
        if (out)
            free(out);
        if (ret != 0) {
