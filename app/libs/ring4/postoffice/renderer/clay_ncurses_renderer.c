 ...
 */
static bool Clay_Ncurses_GetVisibleRect(int x, int y, int w, int h, int *outX, int *outY, int *outW,
                                        int *outH) {
...
static void Clay_Ncurses_RenderText(Clay_RenderCommand *command) {
    ...
    if (!Clay_Ncurses_GetVisibleRect(x, y, textWidth, 1, &dx, &dy, &dw, &dh))
        return;
    ...
    if (command->renderData.text.fontId & CLAY_NCURSES_FONT_BOLD)
        attron(A_BOLD);
    if (command->renderData.text.fontId & CLAY_NCURSES_FONT_UNDERLINE)
        attron(A_UNDERLINE);
    ...
    if (wlen != -1) {
        ...
        for (int k = 0; k < wlen; k++) {
            ...
            currentCols += cw;
            if (currentCols >= skipCols + takeCols)
                break;
        }
...
static void Clay_Ncurses_RenderBorder(Clay_RenderCommand *command) {
    ...
    if (y >= dy && y < dy + dh) {
        ...
        if (x < startX)
            startX++; // Adjust for corner if clipped
        if (x + w - 1 > endX)
            endX--; // Adjust for corner
...
static void Clay_Ncurses_PushScissor(Clay_BoundingBox boundingBox) {
    ...
    float nR = ((next.x + next.width) < (current.x + current.width)) ? (next.x + next.width)
                                                                     : (current.x + current.width);
    float nB = ((next.y + next.height) < (current.y + current.height))
                   ? (next.y + next.height)
...
void Clay_Ncurses_Initialize() {
    ...
    _scissorStack[0] = (Clay_BoundingBox){0, 0, (float)_screenWidth * CLAY_NCURSES_CELL_WIDTH,
                                          (float)_screenHeight * CLAY_NCURSES_CELL_HEIGHT};
...
Clay_Dimensions Clay_Ncurses_GetLayoutDimensions() {
    return (Clay_Dimensions){.width = (float)_screenWidth * CLAY_NCURSES_CELL_WIDTH,
...
Clay_Dimensions Clay_Ncurses_MeasureText(Clay_StringSlice text, Clay_TextElementConfig *config,
    ...
    return (Clay_Dimensions){.width = (float)width * CLAY_NCURSES_CELL_WIDTH,
...
int Clay_Ncurses_ProcessInput(WINDOW *window) {
    ...
    if (key == KEY_MOUSE) {
        ...
        if (getmouse(&event) == OK) {
            ...
            Clay_Vector2 mousePos = {(float)event.x * CLAY_NCURSES_CELL_WIDTH,
                                     (float)event.y * CLAY_NCURSES_CELL_HEIGHT};

            // Persistent state to handle drag/move events where button state might be absent in the
            // event mask
            static bool _isMouseDown = false;
            bool shouldReturnClick = false;

            if (event.bstate &
                (BUTTON1_PRESSED | BUTTON1_DOUBLE_CLICKED | BUTTON1_TRIPLE_CLICKED)) {
                _isMouseDown = true;
 ...
 * Registers a hover callback. The user's function must check `pointerData.state ==
