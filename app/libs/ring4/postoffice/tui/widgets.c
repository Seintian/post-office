...
void tui_widget_destroy(tui_widget_t *widget) {
    ...
    // Prevent freeing a widget that is actively part of the UI tree
    if (widget->parent) {
        return;
    }
...
tui_widget_t *tui_widget_find_at(tui_widget_t *widget, tui_point_t point) {
    if (!widget || !widget->visible || !tui_widget_contains_point(widget, point))
        return NULL;

    // If container, check children in reverse order (top-most first)
    if (widget->type == TUI_WIDGET_PANEL ||
        widget->type == TUI_WIDGET_WINDOW) { // Loose type check for containers
...
void tui_container_add(tui_container_t *container, tui_widget_t *child) {
    ...
    if (container->layout) {
        tui_widget_layout_update((tui_widget_t *)container);
    }
}
...
void tui_container_remove_all(tui_container_t *container) {
    ...
    // Detach all first to allow destroy
...
static bool tui_button_handle_event(tui_widget_t *widget, const tui_event_t *event) {
    ...
    if (event->type == TUI_EVENT_MOUSE) {
        ...
        // Left Click Release (Action) or Click
        if (bstate & BUTTON1_CLICKED) {
             btn->is_pressed = false;
             tui_widget_draw(widget);
             refresh();
             if (btn->callback)
                btn->callback(btn, widget->user_data);
             return true;
        }

        if ((bstate & BUTTON1_RELEASED) && btn->is_pressed) {
            btn->is_pressed = false;
            tui_widget_draw(widget);
            refresh();
            if (btn->callback)
        ...
        // Actually if we just get a non-press event and we were pressed, maybe reset visual state
        // but don't fire? For now, strict Release check is better for "user behavior".
        ...
        if (bstate & (BUTTON3_CLICKED | BUTTON3_RELEASED | BUTTON3_PRESSED)) {
            ...
            if (bstate & (BUTTON3_CLICKED | BUTTON3_RELEASED)) {
                if (btn->on_right_click)
                    btn->on_right_click(btn, widget->user_data);
            }
            return true;
        }
...
#define BUTTON5_PRESSED (1 << 22)
        ...
        if (bstate & BUTTON4_PRESSED) { // Scroll Up
            if (btn->on_scroll)
                btn->on_scroll(btn, 1, widget->user_data);
            return true;
        }
...
void tui_button_set_click_callback(tui_button_t *button, tui_button_click_callback_t callback,
                                   void *user_data) {
    if (button) {
...
void tui_button_set_right_click_callback(tui_button_t *button, tui_button_click_callback_t callback,
                                         void *user_data) {
    if (button) {
        button->on_right_click = callback;
        // user_data is shared in base? careful.
        // Logic usually implies one user_data per widget.
        // If we want distinct user_data for right click, we might need a separate field or just
        // reuse base.user_data. The struct def for callback had 'void* user_data' param in typedef,
        // usually passed from widget->user_data. But here we might overwrite it. Let's assume we
        // stick to one user_data for the widget.
        if (user_data)
            button->base.user_data = user_data;
    }
}

void tui_button_set_scroll_callback(tui_button_t *button,
                                    void (*callback)(tui_button_t *, int, void *),
                                    void *user_data) {
    if (button) {
...
static void tui_panel_free_impl(tui_widget_t *widget) {
    tui_panel_t *p = (tui_panel_t *)widget;
    // Free container resources manually since we can't call free_impl without freeing the struct
    // too early
    tui_container_remove_all(&p->base);
    if (p->base.layout && p->base.layout->free)
        p->base.layout->free(p->base.layout);
...
static void tui_input_draw(tui_widget_t *widget) {
    ...
    for (int i = 0; i < b.size.width; i++)
        mvaddch(b.position.y, b.position.x + i, ' ');
    ...
    if (input->text) {
        ...
        if (draw_len > visible_width)
            draw_len = visible_width;
        if (draw_len < 0)
            draw_len = 0;

        if (draw_len > 0) {
            mvprintw(b.position.y, b.position.x, "%.*s", draw_len,
                     input->text + input->scroll_offset);
        }
    }
...
static void tui_list_draw(tui_widget_t *widget) {
    ...
    if (list->top_visible < 0)
        list->top_visible = 0;

    for (int i = 0; i < max_visible; i++) {
        int item_idx = list->top_visible + i;
        if (item_idx >= list->item_count)
            break;
    ...
    if (list->item_count > max_visible) {
        if (list->top_visible > 0)
            mvaddch(b.position.y, b.position.x + b.size.width - 1, ACS_UARROW);
    ...
    if (event->type == TUI_EVENT_KEY) {
        ...
        if (key == KEY_UP) {
            if (list->selected_index > 0) {
                list->selected_index--;
                if (list->on_select)
                    list->on_select(list, list->selected_index, widget->user_data);
                tui_widget_draw(widget);
                refresh();
            }
            return true;
        ...
        if (event->data.mouse.pressed || (event->data.mouse.button & BUTTON1_CLICKED)) {
...
bool tui_list_add_item(tui_list_t *list, const char *text) {
    ...
    list->items =
        realloc(list->items, sizeof(struct tui_list_item_t *) * (size_t)(list->item_count + 1));
    list->items[list->item_count] = item;
    list->item_count++;

    if (list->selected_index == -1)
        list->selected_index = 0; // Auto select first
...
void tui_list_set_select_callback(tui_list_t *list, tui_list_select_callback_t callback,
                                  void *user_data) {
    if (list) {
...
static void tui_checkbox_draw(tui_widget_t *widget) {
    ...
    char state_char = ' ';
    if (cb->state == 1 || (cb->state == 0 && cb->checked))
        state_char = 'X';
    else if (cb->state == 2)
        state_char = '-'; // Indeterminate
...
static bool tui_checkbox_handle_event(tui_widget_t *widget, const tui_event_t *event) {
    ...
    if (event->type == TUI_EVENT_MOUSE) {
        if (event->data.mouse.button & (BUTTON1_CLICKED | BUTTON1_RELEASED | BUTTON1_PRESSED)) {
             // Only trigger on click or release (if we track press? No, simple logic here)
             // Actually previous logic was:
             // if (event->data.mouse.button & (BUTTON1_CLICKED | BUTTON1_RELEASED))
             // But simpler: just check clicked or pressed if we want instant reaction?
             // Standard UI: usually on Release or Click.
             // Previous code: if (event->data.mouse.button & (BUTTON1_CLICKED | BUTTON1_RELEASED))
             // But if we get CLICKED, we should trigger.
             if (event->data.mouse.button & (BUTTON1_CLICKED | BUTTON1_RELEASED)) {
                if (tui_widget_contains_point(widget, (tui_point_t){(int16_t)event->data.mouse.x,
                                                                    (int16_t)event->data.mouse.y})) {
                    triggered = true;
                }
             }
        }
    ...
    if (triggered) {
        ...
        if (cb->on_toggle)
            cb->on_toggle(cb, cb->checked, widget->user_data);
        tui_widget_draw(widget);
        refresh();
        return true;
    }
...
void tui_checkbox_set_toggle_callback(tui_checkbox_t *checkbox,
                                      tui_checkbox_toggle_callback_t callback, void *user_data) {
    if (checkbox) {
    ...
    if (triggered && !rb->selected) {
        ...
        if (widget->parent && widget->parent->type == TUI_WIDGET_PANEL) { // Check container type
            ...
            while (child) {
                if (child != widget &&
                    child->draw ==
                        tui_radio_draw) { // Hacky type check or use type field if reliable
        ...
        if (rb->on_select)
            rb->on_select(rb, rb->group_id, widget->user_data);
        tui_widget_draw(widget);
        refresh();
        return true;
    }
...
void tui_radio_button_set_select_callback(tui_radio_button_t *radio,
                                          tui_radio_button_select_callback_t callback,
                                          void *user_data) {
    if (radio) {
...
static bool tui_tab_container_handle_event(tui_widget_t *widget, const tui_event_t *event) {
    ...
    if (event->type == TUI_EVENT_MOUSE &&
        (event->data.mouse.button & (BUTTON1_PRESSED | BUTTON1_CLICKED | BUTTON1_RELEASED))) {
                ...
                for (int i = 0; i < tabs->tab_count; i++) {
                    ...
                    if (mx >= curr_x && mx < next_x) {
                        ...
                        if (old_idx >= 0 && old_idx < tabs->tab_count) {
                            ...
                            if (old_content)
                                tui_container_remove(&tabs->base, old_content);
                        }
                        ...
                        tui_widget_draw(widget);
                        clear(); // Hack: Force full clear because switching tabs might leave
                                 // garbage from old tab
                        tui_widget_t *root = tui_get_root();
                        if (root)
                            tui_widget_draw(root); // Redraw everything

                        // Set focus to the new content to ensure keys work immediately
                        if (new_content)
                            tui_set_focus(new_content);
...
static void tui_tab_container_free_impl(tui_widget_t *widget) {
    ...
    for (int i = 0; i < tabs->tab_count; i++) {
        ...
        if (tabs->tabs[i]->content) {
            tabs->tabs[i]->content->parent = NULL; // Detach
            tui_widget_destroy(tabs->tabs[i]->content);
        }
        free(tabs->tabs[i]);
    }
    
    // Free the layout manager allocated in create
    if (tabs->base.layout && tabs->base.layout->free) {
        tabs->base.layout->free(tabs->base.layout);
    }
...
static void tab_layout_fn(tui_layout_manager_t *self, tui_widget_t *container_widget) {
    (void)self;
    if (!container_widget)
        return;
    tui_container_t *c = (tui_container_t *)container_widget;

    // Calculate body bounds based on current container bounds
    tui_rect_t body = container_widget->bounds;
    body.position.y += 2; // Header (1) + Separator (1)
    body.size.height -= 2;
    ...
    tui_widget_t *curr = c->first_child;
    while (curr) {
        tui_widget_set_bounds(curr, body);
        tui_widget_layout_update(curr);
        curr = curr->next;
    }
}

static void tab_layout_free(tui_layout_manager_t *self) {
    if (self)
        free(self);
}
...
tui_tab_container_t *tui_tab_container_create(tui_rect_t bounds) {
    ...
    tui_widget_init(&tabs->base.base, TUI_WIDGET_TAB_CONTAINER);
    ...
    // Assign custom layout manager
    tui_layout_manager_t *lm = malloc(sizeof(tui_layout_manager_t));
    lm->data = NULL;
    lm->layout = tab_layout_fn;
    lm->free = tab_layout_free;
    lm->measure = NULL;
    tabs->base.layout = lm;
...
int tui_tab_container_add_tab(tui_tab_container_t *tabs, const char *title, tui_widget_t *content) {
    ...
    if (tabs->selected_tab == -1) {
        tabs->selected_tab = 0;
        if (content)
            tui_container_add(&tabs->base, content);
    }
...
void tui_list_set_selected_index(struct tui_list_t *list, int index) {
    if (!list) return;
    if (index < -1) index = -1;
    if (index >= list->item_count) index = list->item_count - 1;
    list->selected_index = index;
    // Ensure visibility
    if (list->selected_index >= 0) {
        if (list->selected_index < list->top_visible) {
            list->top_visible = list->selected_index;
        } else if (list->selected_index >= list->top_visible + list->visible_items) {
             list->top_visible = list->selected_index - list->visible_items + 1;
        }
    }
}
...
void tui_tab_container_set_selected_tab(struct tui_tab_container_t *tabs, int index) {
    if (!tabs) return;
    if (index < 0 || index >= tabs->tab_count) return;
    if (tabs->selected_tab == index) return;
    ...
    tui_widget_t *new_content = tabs->tabs[index]->content;
    if (new_content) {
        tui_container_add(&tabs->base, new_content);
        tui_set_focus(new_content);
    }
