...
static int             g_mute_console = 0; // suppress console during tests or on demand
...
static void record_format_line(const log_record_t *r, char *out, size_t outsz) {
    ...
    snprintf(out, outsz, "%s.%06ld %llu %-5s %s:%d %s() - %s\n",
             tsbuf, usec, (unsigned long long)r->tid, lvlstr[r->level],
...
static void write_record(const log_record_t *r) {
    ...
    if ((g_sinks_mask & LOGGER_SINK_CONSOLE) && !g_mute_console) {
...
int logger_init(const logger_config *cfg) {
    ...
    if (g_fp) { fclose(g_fp); }
    g_fp = NULL;
    __atomic_store_n(&g_dropped_new, 0ul, __ATOMIC_RELAXED);
    __atomic_store_n(&g_overwritten_old, 0ul, __ATOMIC_RELAXED);
    ...
    if (!g_free) {
        free(g_pool); g_pool=NULL; g_pool_n=0;
        perf_ringbuf_destroy(&g_ring);
        return -1;
    }
    ...
    // allow test harness to mute console to keep CI output clean
    const char *mute = getenv("PO_TEST_MUTE_CONSOLE");
    g_mute_console = (mute && (*mute == '1')) ? 1 : 0;
...
void logger_shutdown(void) {
    ...
    free(g_workers); g_workers=NULL; g_nworkers=0;
    perf_ringbuf_destroy(&g_ring);
...
int logger_set_level(logger_level_t level) {
    if (level < LOG_TRACE || level > LOG_FATAL) { errno = EINVAL; return -1; }
...
static void enqueue_record(log_record_t *rec) {
    ...
    if (g_policy == LOGGER_DROP_NEW) {
        ...
        __atomic_add_fetch(&g_dropped_new, 1, __ATOMIC_RELAXED);
    ...
    if (perf_ringbuf_dequeue(g_ring, &old) == 0) {
        __atomic_add_fetch(&g_overwritten_old, 1, __ATOMIC_RELAXED);
...
void logger_logv(logger_level_t level,
                 ...
                 const char *fmt,
                 va_list ap) {
    if (!g_ring) return; // not initialized yet
    ...
    if (perf_ringbuf_dequeue(g_free, &ptr) != 0) {
        ...
        unsigned long dropped = __atomic_add_fetch(&g_dropped_new, 1, __ATOMIC_RELAXED);
        ...
        if (should_emit_overflow_notice(dropped)) {
            ...
            unsigned long overwritten = __atomic_load_n(&g_overwritten_old, __ATOMIC_RELAXED);
            snprintf(w2.msg, sizeof(w2.msg),
                     "logger overflow: dropped_new=%lu overwritten_old=%lu (policy=%s)",
                     dropped, overwritten,
                     g_policy == LOGGER_DROP_NEW ? "DROP_NEW" : "OVERWRITE_OLDEST");
            write_record(&w2);
        }
        return;
    }
    ...
    if (file) {
        ...
        memcpy(r->file, file + (n >= sizeof(r->file) ? n-(sizeof(r->file)-1) : 0), n >= sizeof(r->file) ? sizeof(r->file)-1 : n);
    ...
    unsigned long dropped = __atomic_load_n(&g_dropped_new, __ATOMIC_RELAXED);
    unsigned long overwritten = __atomic_load_n(&g_overwritten_old, __ATOMIC_RELAXED);
...
void logger_log(logger_level_t level,
                ...
                const char *fmt,
                ...) {
    if ((level < (logger_level_t)LOGGER_COMPILE_LEVEL) || (level < (logger_level_t)_logger_runtime_level)) return;
