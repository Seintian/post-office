...
static void tui_table_draw(tui_widget_t* widget) {
    ...
    for(int i=0; i<table->column_count; i++) {
        if(table->columns[i].weight <= 0) total_fixed += table->columns[i].width;
        else total_weight += table->columns[i].weight;
    }
    ...
    if (table->show_headers) {
        mvhline(y, x, 0, b.size.width); // Header bg?
        int curr_x = x;
        if(table->show_grid) mvaddch(y, curr_x++, ACS_VLINE);
        ...
        // Separator line
        if(table->show_grid) {
             mvhline(y, x, ACS_HLINE, b.size.width);
             y++;
        }
    }
    ...
    // Scroll
    if (table->selected_row < table->top_visible_row) table->top_visible_row = table->selected_row;
    if (table->selected_row >= table->top_visible_row + visible_h) table->top_visible_row = table->selected_row - visible_h + 1;
    if (table->top_visible_row < 0) table->top_visible_row = 0;
    
    for(int r=0; r < visible_h; r++) {
        int row_idx = table->top_visible_row + r;
        if (row_idx >= table->row_count) break;
        
        int curr_x = x;
        int attrs = 0;
        if (row_idx == table->selected_row) {
            attrs |= (int)A_REVERSE;
            if (widget->has_focus) attrs |= (int)A_BOLD;
        }
        
        if(table->show_grid) mvaddch(y+r, curr_x++, ACS_VLINE);
    ...
    if (event->type == TUI_EVENT_KEY) {
        ...
        if (key == KEY_UP) {
            if (table->selected_row > 0) {
                table->selected_row--;
                if(table->on_select) table->on_select(table, table->selected_row, widget->user_data);
...
static void tui_table_free_impl(tui_widget_t* widget) {
    tui_table_t* table = (tui_table_t*)widget;
    for(int i=0; i<table->column_count; i++) free(table->columns[i].title);
    free(table->columns);
    
    for(int i=0; i<table->row_count; i++) {
        for(int c=0; c<table->column_count; c++) free(table->rows[i][c]);
        free(table->rows[i]);
    }
    free(table->rows);
    free(table);
...
void tui_table_add_column(tui_table_t* table, const char* title, int width, float weight) {
    if(!table) return;
    table->columns = realloc(table->columns, sizeof(tui_table_column_t) * (size_t)(table->column_count + 1));
