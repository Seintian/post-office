...
static int write_full(int fd, const void *buf, size_t len) {
	...
	while (nleft > 0) {
		...
		if (n > 0) { p += n; nleft -= (size_t)n; continue; }
...
int framing_write_msg(int fd, const po_header_t *header_net, const uint8_t *payload, uint32_t payload_len) {
	if (payload_len > g_max_payload) { errno = EMSGSIZE; return -1; }
	...
	struct iovec iov[3];
	int iovcnt = 0;
	iov[iovcnt].iov_base = &len_be; iov[iovcnt].iov_len = sizeof(len_be); iovcnt++;
	// cast away const for writev API; buffers aren't modified by kernel
	iov[iovcnt].iov_base = (void*)(uintptr_t)header_net; iov[iovcnt].iov_len = sizeof(po_header_t); iovcnt++;
	if (payload_len && payload) { iov[iovcnt].iov_base = (void*)(uintptr_t)payload; iov[iovcnt].iov_len = payload_len; iovcnt++; }

	// Try writev once; if partial, fallback to write_full on a temp linear buffer
	ssize_t n = writev(fd, iov, iovcnt);
	if (n < 0) {
		if (errno == EINTR) return framing_write_msg(fd, header_net, payload, payload_len);
		return -1;
	}
...
int framing_write_zcp(int fd, const po_header_t *header_net, const zcp_buffer_t *payload_buf __attribute__((unused))) {
	// We don't know the payload size from the opaque type; callers should use contiguous payload variant
	// For compatibility, treat as zero-length payload here.
...
int framing_read_msg(int fd, po_header_t *header_out, zcp_buffer_t **payload_out) {
	...
	if (rc != 0) return rc; // -1 error, -2 EOF
	uint32_t total = ntohl(len_be);
	if (total < sizeof(po_header_t)) { errno = EPROTO; return -1; }
	...
	*header_out = net_hdr;
	...
	if (header_out->version != PROTOCOL_VERSION) { errno = EPROTONOSUPPORT; return -1; }
