...
static int cmp_bin(const void *a, const void *b) {
	...
	if (ea->keylen != eb->keylen) return 1;
...
po_index_t *po_index_create(size_t expected_entries) {
	...
	if (!idx) return NULL;
	idx->ht = hashtable_create_sized(cmp_bin, hash_bin, expected_entries ? expected_entries : 128);
	if (!idx->ht) { free(idx); return NULL; }
	return idx;
}
...
void po_index_destroy(po_index_t **pidx) {
	if (!*pidx) return;
	po_index_t *idx = *pidx;
	hashtable_iter_t *it = hashtable_iterator(idx->ht);
	while (it && hashtable_iter_next(it)) {
		idx_entry_t *e = hashtable_iter_key(it);
		free(e->key);
		free(e);
	}
	free(it);
	hashtable_destroy(&idx->ht);
	free(idx);
	*pidx = NULL;
}

int po_index_put(po_index_t *idx, const void *key, size_t keylen, uint64_t offset, uint32_t len) {
	if (!idx || !key || keylen == 0) return -1;
	idx_entry_t lookup = {.key = (void *)key, .keylen = keylen};
	idx_entry_t *existing = hashtable_get(idx->ht, &lookup);
	if (existing) {
	...
	e->offset = offset;
	e->len = len;
	return hashtable_put(idx->ht, e, e) == 1 ? 0 : 0;
}
...
int po_index_get(const po_index_t *idx, const void *key, size_t keylen,
	...
	if (!idx || !key || keylen == 0) return -1;
	idx_entry_t lookup = {.key = (void *)key, .keylen = keylen};
	idx_entry_t *e = hashtable_get(idx->ht, &lookup);
	if (!e) return -1;
