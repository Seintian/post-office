...
static atomic_int sampler_running = 0;
...
static int read_proc_stat(struct proc_stat_sample *s) {
    ...
    if (getline(&line, &n, f) > 0) {
        ...
        // guest_nice
        unsigned long long user, nice, system, idlev, iowait, irq, softirq, steal, guest,
            guest_nice;
        int matched =
            sscanf(line, "cpu  %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu", &user, &nice,
                   &system, &idlev, &iowait, &irq, &softirq, &steal, &guest, &guest_nice);
        if (matched >= 8) {
...
static void *sampler_loop(void *arg) {
    ...
    while (1) {
        if (!atomic_load_explicit(&sampler_running, memory_order_acquire))
            break;
        ...
        if (b.iowait >= a.iowait)
            iowait_delta = b.iowait - a.iowait;
...
int po_sysinfo_sampler_get(double *cpu_util_pct, double *cpu_iowait_pct) {
    ...
    pthread_mutex_lock(&sampler_lock);
    if (cpu_util_pct)
        *cpu_util_pct = cached_cpu_util;
    if (cpu_iowait_pct)
        *cpu_iowait_pct = cached_cpu_iowait;
    pthread_mutex_unlock(&sampler_lock);
    return 0;
}
