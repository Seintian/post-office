...
int main(int argc, char *argv[]) {
    ...
    if (po_logger_init(&(po_logger_config_t){.level = po_logger_level_from_str(loglevel) == -1
                                             ...
                                             .consumers = 1}) != 0)
        return 1;
    po_logger_add_sink_file("logs/ticket_issuer.log", true);
    ...
    char sock_dir[512];
    if (user_home) {
        snprintf(sock_dir, sizeof(sock_dir), "%s/.postoffice", user_home);
        snprintf(sock_path, sizeof(sock_path), "%s/issuer.sock", sock_dir);
        // Ensure directory exists
        struct stat st = {0};
        if (stat(sock_dir, &st) == -1) {
            if (mkdir(sock_dir, 0700) != 0 && errno != EEXIST) {
                LOG_ERROR("Failed to create directory %s: %s", sock_dir, strerror(errno));
            }
        }
    } else {
    ...
    LOG_DEBUG("TI: Unlinking %s", sock_path);
    unlink(sock_path);
    LOG_DEBUG("TI: Listening...");
    int fd = po_socket_listen_unix(sock_path, 128);
    LOG_DEBUG("TI: Listen returned %d", fd);
    if (fd < 0) {
        LOG_FATAL("Failed to bind Ticket Issuer socket: %s (Error: %s)", sock_path,
                  strerror(errno));
        return 1;
    }
    LOG_INFO("Ticket Issuer: Created and listening on %s (fd=%d)", sock_path, fd);
    LOG_DEBUG("TI: Init threadpool");

    threadpool_t *tp = tp_create(pool_size, 4096);
    LOG_DEBUG("TI: TP Created %p", (void *)tp);
    if (!tp) {
    ...
    while (!g_shutdown) {
        ...
        LOG_TRACE("Entering poller_wait...");
            ...
            if (ev[i].data.fd == fd) {
                ...
                while (!g_shutdown) {
                    ...
                    if (ctx) {
                        ...
                        if (ret != 0) {
                            LOG_ERROR("Failed to submit client_fd=%d to thread pool (queue full?)",
                                      client);
                            free(ctx);
