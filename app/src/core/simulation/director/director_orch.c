...
static void launch_process(const char *bin, char *const argv[]) {
    ...
    if (pid == 0) {
        ...
        if (g_pids) po_vector_push(g_pids, (void*)(intptr_t)pid);
    }
}
...
void spawn_simulation_subsystems(const director_config_t *cfg) {
    ...
    char *args_ti[] = {
        "bin/post_office_ticket_issuer", 
        "-l", (char *)cfg->log_level,
        "--pool-size", pool_str, 
        NULL
    };
    ...
    char *args_um[] = {
        "bin/post_office_users_manager", 
        "-l", (char *)cfg->log_level,
        "--initial", init_str,
        "--batch", batch_str,
        "--pool-size", um_pool_str,
        NULL
    };
