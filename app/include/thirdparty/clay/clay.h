...
#define CLAY_TEXT_CONFIG(...)                                                                      \
    Clay__StoreTextElementConfig(CLAY__CONFIG_WRAPPER(Clay_TextElementConfig, __VA_ARGS__))
...
#define CLAY_SIZING_FIT(...)                                                                       \
    (CLAY__INIT(Clay_SizingAxis){.size = {.minMax = {__VA_ARGS__}}, .type = CLAY__SIZING_TYPE_FIT})

#define CLAY_SIZING_GROW(...)                                                                      \
    (CLAY__INIT(Clay_SizingAxis){.size = {.minMax = {__VA_ARGS__}}, .type = CLAY__SIZING_TYPE_GROW})
...
// Note: If a compile error led you here, you might be trying to use CLAY_ID with something other
// than a string literal. To construct an ID with a dynamic string, use CLAY_SID instead.
#define CLAY_ID(label) CLAY_SID(CLAY_STRING(label))

#define CLAY_SID(label) Clay__HashString(label, 0)

// Note: If a compile error led you here, you might be trying to use CLAY_IDI with something other
// than a string literal. To construct an ID with a dynamic string, use CLAY_SIDI instead.
...
// Note: If a compile error led you here, you might be trying to use CLAY_ID_LOCAL with something
// other than a string literal. To construct an ID with a dynamic string, use CLAY_SID_LOCAL
// instead.
...
// Note: If a compile error led you here, you might be trying to use CLAY_IDI_LOCAL with something
// other than a string literal. To construct an ID with a dynamic string, use CLAY_SIDI_LOCAL
// instead.
...
// Note: If an error led you here, it's because CLAY_STRING can only be used with string literals,
// i.e. CLAY_STRING("SomeString") and not CLAY_STRING(yourString)
#define CLAY_STRING(string)                                                                        \
    (CLAY__INIT(Clay_String){.isStaticallyAllocated = true,                                        \
...
// GCC marks the above CLAY__ELEMENT_DEFINITION_LATCH as an unused variable for files that include
// clay.h but don't declare any layout This is to suppress that warning
...
/* This macro looks scary on the surface, but is actually quite simple.
  ...
  The for loop will only ever run a single iteration, putting Clay__CloseElement() in the increment
  of the loop means that it will run after the body - where the children are declared. It just
  exists to make sure you don't forget to call Clay_CloseElement().
*/
#define CLAY_AUTO_ID(...)                                                                          \
    for (CLAY__ELEMENT_DEFINITION_LATCH = (Clay__OpenElement(),                                    \
                                           Clay__ConfigureOpenElement(CLAY__CONFIG_WRAPPER(        \
                                               Clay_ElementDeclaration, __VA_ARGS__)),             \
                                           0);                                                     \
         CLAY__ELEMENT_DEFINITION_LATCH < 1;                                                       \
         CLAY__ELEMENT_DEFINITION_LATCH = 1, Clay__CloseElement())

#define CLAY(id, ...)                                                                              \
    for (CLAY__ELEMENT_DEFINITION_LATCH = (Clay__OpenElementWithId(id),                            \
                                           Clay__ConfigureOpenElement(CLAY__CONFIG_WRAPPER(        \
                                               Clay_ElementDeclaration, __VA_ARGS__)),             \
                                           0);                                                     \
         CLAY__ELEMENT_DEFINITION_LATCH < 1;                                                       \
...
// These macros exist to allow the CLAY() macro to be called both with an inline struct definition,
// such as CLAY({ .id = something... }); As well as by passing a predefined declaration struct
...
// Note: Clay_String is not guaranteed to be null terminated. It may be if created from a literal C
// string, but it is also used to represent slices.
typedef struct Clay_String {
    // Set this boolean to true if the char* data underlying this string will live for the entire
    // lifetime of the program. This will automatically be set for strings created with CLAY_STRING,
    // as the macro requires a string literal.
    bool isStaticallyAllocated;
    int32_t length;
    // The underlying character memory. Note: this will not be copied and will not extend the
...
// Internally clay conventionally represents colors as 0-255, but interpretation is up to the
// renderer.
...
// Represents a hashed string ID used for identifying and finding specific clay UI elements,
// required by functions such as Clay_PointerOver() and Clay_GetElementData().
typedef struct Clay_ElementId {
    ...
    uint32_t baseId; // A base hash value to start from, for example the parent element ID is used
                     // when calculating CLAY_ID_LOCAL().
...
// The rounding is determined by drawing a circle inset into the element corner by (radius, radius)
// pixels.
...
typedef CLAY_PACKED_ENUM{
    // (Default) Aligns child elements to the left hand side of this element, offset by
    // padding.width.left
    CLAY_ALIGN_X_LEFT,
...
typedef CLAY_PACKED_ENUM{
    ...
    // Expands along this axis to fill available space in the parent element, sharing it with other
    // GROW elements.
    CLAY__SIZING_TYPE_GROW,
    // Expects 0-1 range. Clamps the axis size to a percent of the parent container's axis size
...
// Controls the minimum and maximum size in pixels that this element is allowed to grow or shrink
// to, overriding sizing types such as FIT or GROW.
...
typedef struct Clay_SizingAxis {
    union {
        Clay_SizingMinMax
            minMax; // Controls the minimum and maximum size in pixels that this element is allowed
                    // to grow or shrink to, overriding sizing types such as FIT or GROW.
        float percent; // Expects 0-1 range. Clamps the axis size to a percent of the parent
...
// Controls "padding" in pixels, which is a gap between the bounding box of this element and where
...
// Controls various settings that affect the size and position of an element, as well as the sizes
// and positions of any child elements.
typedef struct Clay_LayoutConfig {
    Clay_Sizing sizing;   // Controls the sizing of this element inside it's parent container,
                          // including FIT, GROW, PERCENT and FIXED sizing.
    Clay_Padding padding; // Controls "padding" in pixels, which is a gap between the bounding box
                          // of this element and where its children will be placed.
    uint16_t childGap;    // Controls the gap in pixels between child elements along the layout axis
                          // (horizontal gap for LEFT_TO_RIGHT, vertical gap for TOP_TO_BOTTOM).
    Clay_ChildAlignment childAlignment;   // Controls how child elements are aligned on each axis.
    Clay_LayoutDirection layoutDirection; // Controls the direction in which child elements will be
...
// Controls how text "wraps", that is how it is broken into multiple lines when there is
...
typedef CLAY_PACKED_ENUM{
    // (default) Horizontally aligns wrapped lines of text to the left hand side of their bounding
    // box.
...
typedef struct Clay_TextElementConfig {
    ...
    uint16_t fontSize;
    // Controls extra horizontal spacing between characters. Handled by the function provided to
    // Clay_MeasureText.
    uint16_t letterSpacing;
    // Controls additional vertical space between wrapped lines of text.
    uint16_t lineHeight;
    // Controls how text "wraps", that is how it is broken into multiple lines when there is
    // insufficient horizontal space. CLAY_TEXT_WRAP_WORDS (default) breaks on whitespace
    // characters. CLAY_TEXT_WRAP_NEWLINES doesn't break on space characters, only on newlines.
    // CLAY_TEXT_WRAP_NONE disables wrapping entirely.
    Clay_TextElementConfigWrapMode wrapMode;
    // Controls how wrapped lines of text are horizontally aligned within the outer text bounding
    // box. CLAY_TEXT_ALIGN_LEFT (default) - Horizontally aligns wrapped lines of text to the left
    // hand side of their bounding box. CLAY_TEXT_ALIGN_CENTER - Horizontally aligns wrapped lines
    // of text to the center of their bounding box. CLAY_TEXT_ALIGN_RIGHT - Horizontally aligns
...
typedef struct Clay_AspectRatioElementConfig {
    float aspectRatio; // A float representing the target "Aspect ratio" for an element, which is
                       // its final width divided by its final height.
...
// Note: see https://github.com/user-attachments/assets/b8c6dfaa-c1b1-41a4-be55-013473e4a6ce for a
// visual explanation.
typedef CLAY_PACKED_ENUM{
...
typedef struct Clay_FloatingAttachPoints {
    Clay_FloatingAttachPointType
        element; // Controls the origin point on a floating element that attaches to its parent.
    Clay_FloatingAttachPointType parent; // Controls the origin point on the parent element that the
                                         // floating element attaches to.
} Clay_FloatingAttachPoints;

// Controls how mouse pointer events like hover and click are captured or passed through to elements
// underneath a floating element.
typedef CLAY_PACKED_ENUM{
    // (default) "Capture" the pointer event and don't allow events like hover and click to pass
    // through to elements underneath.
    CLAY_POINTER_CAPTURE_MODE_CAPTURE,
    //    CLAY_POINTER_CAPTURE_MODE_PARENT, TODO pass pointer through to attached parent

    // Transparently pass through pointer events like hover and click to elements underneath the
    // floating element.
...
typedef CLAY_PACKED_ENUM{
    ...
    CLAY_ATTACH_TO_NONE,
    // Attaches this floating element to its parent, positioned based on the .attachPoints and
    // .offset fields.
    CLAY_ATTACH_TO_PARENT,
    // Attaches this floating element to an element with a specific ID, specified with the .parentId
    // field. positioned based on the .attachPoints and .offset fields.
    CLAY_ATTACH_TO_ELEMENT_WITH_ID,
    // Attaches this floating element to the root of the layout, which combined with the .offset
    // field provides functionality similar to "absolute positioning".
    CLAY_ATTACH_TO_ROOT,
} Clay_FloatingAttachToElement;

// Controls whether or not a floating element is clipped to the same clipping rectangle as the
// element it's attached to.
typedef CLAY_PACKED_ENUM{// (default) - The floating element does not inherit clipping.
                         CLAY_CLIP_TO_NONE,
...
// Controls various settings related to "floating" elements, which are elements that "float" above
// other elements, potentially overlapping their boundaries, and not affecting the layout of sibling
// or parent elements.
typedef struct Clay_FloatingElementConfig {
    ...
    // When used in conjunction with .attachTo = CLAY_ATTACH_TO_ELEMENT_WITH_ID, attaches this
    // floating element to the element in the hierarchy with the provided ID. Hint: attach the ID to
    // the other element with .id = CLAY_ID("yourId"), and specify the id the same way, with
    // .parentId = CLAY_ID("yourId").id
    uint32_t parentId;
    // Controls the z index of this floating element and all its children. Floating elements are
    // sorted in ascending z order before output. zIndex is also passed to the renderer for all
    // elements contained within this floating element.
    int16_t zIndex;
    // Controls how mouse pointer events like hover and click are captured or passed through to
    // elements underneath / behind a floating element. Enum is of the form
    // CLAY_ATTACH_POINT_foo_bar. See Clay_FloatingAttachPoints for more details. Note: see <img
    // src="https://github.com/user-attachments/assets/b8c6dfaa-c1b1-41a4-be55-013473e4a6ce /> and
    // <img src="https://github.com/user-attachments/assets/ebe75e0d-1904-46b0-982d-418f929d1516 />
    // for a visual explanation.
    Clay_FloatingAttachPoints attachPoints;
    // Controls how mouse pointer events like hover and click are captured or passed through to
    // elements underneath a floating element. CLAY_POINTER_CAPTURE_MODE_CAPTURE (default) -
    // "Capture" the pointer event and don't allow events like hover and click to pass through to
    // elements underneath. CLAY_POINTER_CAPTURE_MODE_PASSTHROUGH - Transparently pass through
    // pointer events like hover and click to elements underneath the floating element.
    Clay_PointerCaptureMode pointerCaptureMode;
    // Controls which element a floating element is "attached" to (i.e. relative offset from).
    // CLAY_ATTACH_TO_NONE (default) - Disables floating for this element.
    // CLAY_ATTACH_TO_PARENT - Attaches this floating element to its parent, positioned based on the
    // .attachPoints and .offset fields. CLAY_ATTACH_TO_ELEMENT_WITH_ID - Attaches this floating
    // element to an element with a specific ID, specified with the .parentId field. positioned
    // based on the .attachPoints and .offset fields. CLAY_ATTACH_TO_ROOT - Attaches this floating
    // element to the root of the layout, which combined with the .offset field provides
    // functionality similar to "absolute positioning".
    Clay_FloatingAttachToElement attachTo;
    // Controls whether or not a floating element is clipped to the same clipping rectangle as the
    // element it's attached to. CLAY_CLIP_TO_NONE (default) - The floating element does not inherit
    // clipping. CLAY_CLIP_TO_ATTACHED_PARENT - The floating element is clipped to the same clipping
...
// Controls the axis on which an element switches to "scrolling", which clips the contents and
// allows scrolling in that direction.
typedef struct Clay_ClipElementConfig {
    ...
    Clay_Vector2 childOffset; // Offsets the x,y positions of all child elements. Used primarily for
...
typedef struct Clay_BorderWidth {
    ...
    // e.g. for LEFT_TO_RIGHT, borders will be vertical lines, and for TOP_TO_BOTTOM borders will be
    // horizontal lines. .betweenChildren borders will result in individual RECTANGLE render
...
typedef struct Clay_BorderElementConfig {
    Clay_Color color;       // Controls the color of all borders with width > 0. Conventionally
                            // represented as 0-255, but interpretation is up to the renderer.
    Clay_BorderWidth width; // Controls the widths of individual borders. At least one of these
...
typedef struct Clay_TextRenderData {
    ...
    // Conventionally represented as 0-255 for each channel, but interpretation is up to the
    // renderer.
    Clay_Color textColor;
    // An integer representing the font to use to render this text, transparently passed through
    // from the text declaration.
    uint16_t fontId;
    uint16_t fontSize;
...
typedef struct Clay_RectangleRenderData {
    // The solid background color to fill this rectangle with. Conventionally represented as 0-255
    // for each channel, but interpretation is up to the renderer.
    Clay_Color backgroundColor;
    // Controls the "radius", or corner rounding of elements, including rectangles, borders and
    // images. The rounding is determined by drawing a circle inset into the element corner by
...
typedef struct Clay_ImageRenderData {
    // The tint color for this image. Note that the default value is 0,0,0,0 and should likely be
    // interpreted as "untinted". Conventionally represented as 0-255 for each channel, but
    // interpretation is up to the renderer.
    Clay_Color backgroundColor;
    // Controls the "radius", or corner rounding of this image.
    // The rounding is determined by drawing a circle inset into the element corner by (radius,
    // radius) pixels.
    Clay_CornerRadius cornerRadius;
    // A pointer transparently passed through from the original element definition, typically used
    // to represent image data.
    void *imageData;
...
// Render command data when commandType == CLAY_RENDER_COMMAND_TYPE_SCISSOR_START || commandType ==
...
typedef struct Clay_ScrollContainerData {
    // Note: This is a pointer to the real internal scroll position, mutating it may cause a change
    // in final layout. Intended for use with external functionality that modifies scroll position,
    // such as scroll bars or auto scrolling.
    Clay_Vector2 *scrollPosition;
    // The bounding box of the scroll element.
    Clay_Dimensions scrollContainerDimensions;
    // The outer dimensions of the inner scroll container content, including the padding of the
    // parent scroll container.
    Clay_Dimensions contentDimensions;
    // The config that was originally passed to the clip element.
    Clay_ClipElementConfig config;
    // Indicates whether an actual scroll container matched the provided ID or if the default struct
    // was returned.
    bool found;
...
typedef struct Clay_ElementData {
    // The rectangle that encloses this UI element, with the position relative to the root of the
    // layout.
    Clay_BoundingBox boundingBox;
    // Indicates whether an actual Element matched the provided ID or if the default struct was
    // returned.
    bool found;
} Clay_ElementData;
...
typedef CLAY_PACKED_ENUM{
    ...
    // The renderer should begin clipping all future draw commands, only rendering content that
    // falls within the provided boundingBox.
    CLAY_RENDER_COMMAND_TYPE_SCISSOR_START,
    // The renderer should finish any previously active clipping, and begin rendering elements in
    // full again.
    CLAY_RENDER_COMMAND_TYPE_SCISSOR_END,
    // The renderer should provide a custom implementation for handling this render command based on
    // its .customData
    CLAY_RENDER_COMMAND_TYPE_CUSTOM,
} Clay_RenderCommandType;

typedef struct Clay_RenderCommand {
    // A rectangular box that fully encloses this UI element, with the position relative to the root
    // of the layout.
    Clay_BoundingBox boundingBox;
    // A struct union containing data specific to this command's commandType.
    ...
    // Note: the render command array is already sorted in ascending order, and will produce correct
    // results if drawn in naive order. This field is intended for use in batching renderers for
    ...
    // CLAY_RENDER_COMMAND_TYPE_BORDER - The renderer should draw a colored border inset into the
    // bounding box. CLAY_RENDER_COMMAND_TYPE_TEXT - The renderer should draw text.
    // CLAY_RENDER_COMMAND_TYPE_IMAGE - The renderer should draw an image.
    // CLAY_RENDER_COMMAND_TYPE_SCISSOR_START - The renderer should begin clipping all future draw
    // commands, only rendering content that falls within the provided boundingBox.
    // CLAY_RENDER_COMMAND_TYPE_SCISSOR_END - The renderer should finish any previously active
    // clipping, and begin rendering elements in full again. CLAY_RENDER_COMMAND_TYPE_CUSTOM - The
    // renderer should provide a custom implementation for handling this render command based on its
    // .customData
...
typedef CLAY_PACKED_ENUM{
    ...
    // The left mouse button click or touch happened at some point in the past, and is still
    // currently held down this frame.
    CLAY_POINTER_DATA_PRESSED,
    // The left mouse button click or touch was released this frame.
    CLAY_POINTER_DATA_RELEASED_THIS_FRAME,
    // The left mouse button click or touch is not currently down / was released at some point in
    // the past.
...
typedef struct Clay_PointerData {
    ...
    // CLAY_POINTER_DATA_PRESSED - The left mouse button click or touch happened at some point in
    // the past, and is still currently held down this frame. CLAY_POINTER_DATA_RELEASED_THIS_FRAME
    // - The left mouse button click or touch was released this frame. CLAY_POINTER_DATA_RELEASED -
    // The left mouse button click or touch is not currently down / was released at some point in
    // the past.
...
typedef struct Clay_ElementDeclaration {
    // Controls various settings that affect the size and position of an element, as well as the
    // sizes and positions of any child elements.
    Clay_LayoutConfig layout;
    // Controls the background color of the resulting element.
    // By convention specified as 0-255, but interpretation is up to the renderer.
    // If no other config is specified, .backgroundColor will generate a RECTANGLE render command,
    // otherwise it will be passed as a property to IMAGE or CUSTOM render commands.
    Clay_Color backgroundColor;
    // Controls the "radius", or corner rounding of elements, including rectangles, borders and
    // images.
    Clay_CornerRadius cornerRadius;
    ...
    // Controls whether and how an element "floats", which means it layers over the top of other
    // elements in z order, and doesn't affect the position and size of siblings or parent elements.
    // Note: in order to activate floating, .floating.attachTo must be set to something other than
    // the default value.
    Clay_FloatingElementConfig floating;
    // Used to create CUSTOM render commands, usually to render element types not supported by Clay.
    Clay_CustomElementConfig custom;
    // Controls whether an element should clip its contents, as well as providing child x,y offset
...
typedef CLAY_PACKED_ENUM{
    // A text measurement function wasn't provided using Clay_SetMeasureTextFunction(), or the
    // provided function was null.
    CLAY_ERROR_TYPE_TEXT_MEASUREMENT_FUNCTION_NOT_PROVIDED,
    // Clay attempted to allocate its internal data structures but ran out of space.
    // The arena passed to Clay_Initialize was created with a capacity smaller than that required by
    // Clay_MinMemorySize().
    CLAY_ERROR_TYPE_ARENA_CAPACITY_EXCEEDED,
    // Clay ran out of capacity in its internal array for storing elements. This limit can be
    // increased with Clay_SetMaxElementCount().
    ...
    // A floating element was declared using CLAY_ATTACH_TO_ELEMENT_ID and either an invalid
    // .parentId was provided or no element with the provided .parentId was found.
    CLAY_ERROR_TYPE_FLOATING_CONTAINER_PARENT_NOT_FOUND,
    // An element was declared that using CLAY_SIZING_PERCENT but the percentage value was over 1.
    // Percentage values are expected to be in the 0-1 range.
    CLAY_ERROR_TYPE_PERCENTAGE_OVER_1,
    // Clay encountered an internal error. It would be wonderful if you could report this so we can
    // fix it!
    CLAY_ERROR_TYPE_INTERNAL_ERROR,
    // Clay__OpenElement was called more times than Clay__CloseElement, so there were still
    // remaining open elements when the layout ended.
...
typedef struct Clay_ErrorData {
    // Represents the type of error clay encountered while computing layout.
    // CLAY_ERROR_TYPE_TEXT_MEASUREMENT_FUNCTION_NOT_PROVIDED - A text measurement function wasn't
    // provided using Clay_SetMeasureTextFunction(), or the provided function was null.
    // CLAY_ERROR_TYPE_ARENA_CAPACITY_EXCEEDED - Clay attempted to allocate its internal data
    // structures but ran out of space. The arena passed to Clay_Initialize was created with a
    // capacity smaller than that required by Clay_MinMemorySize().
    // CLAY_ERROR_TYPE_ELEMENTS_CAPACITY_EXCEEDED - Clay ran out of capacity in its internal array
    // for storing elements. This limit can be increased with Clay_SetMaxElementCount().
    // CLAY_ERROR_TYPE_TEXT_MEASUREMENT_CAPACITY_EXCEEDED - Clay ran out of capacity in its internal
    // array for storing elements. This limit can be increased with
    // Clay_SetMaxMeasureTextCacheWordCount(). CLAY_ERROR_TYPE_DUPLICATE_ID - Two elements were
    // declared with exactly the same ID within one layout.
    // CLAY_ERROR_TYPE_FLOATING_CONTAINER_PARENT_NOT_FOUND - A floating element was declared using
    // CLAY_ATTACH_TO_ELEMENT_ID and either an invalid .parentId was provided or no element with the
    // provided .parentId was found. CLAY_ERROR_TYPE_PERCENTAGE_OVER_1 - An element was declared
    // that using CLAY_SIZING_PERCENT but the percentage value was over 1. Percentage values are
    // expected to be in the 0-1 range. CLAY_ERROR_TYPE_INTERNAL_ERROR - Clay encountered an
    // internal error. It would be wonderful if you could report this so we can fix it!
...
// Returns the size, in bytes, of the minimum amount of memory Clay requires to operate at its
// current settings.
CLAY_DLL_EXPORT uint32_t Clay_MinMemorySize(void);
// Creates an arena for clay to use for its internal allocations, given a certain capacity in bytes
// and a pointer to an allocation of at least that size. Intended to be used with Clay_MinMemorySize
// in the following way: uint32_t minMemoryRequired = Clay_MinMemorySize(); Clay_Arena clayMemory =
// Clay_CreateArenaWithCapacityAndMemory(minMemoryRequired, malloc(minMemoryRequired));
CLAY_DLL_EXPORT Clay_Arena Clay_CreateArenaWithCapacityAndMemory(size_t capacity, void *memory);
// Sets the state of the "pointer" (i.e. the mouse or touch) in Clay's internal data. Used for
// detecting and responding to mouse events in the debug view, as well as for Clay_Hovered() and
// scroll element handling.
CLAY_DLL_EXPORT void Clay_SetPointerState(Clay_Vector2 position, bool pointerDown);
// Initialize Clay's internal arena and setup required data before layout can begin. Only needs to
// be called once.
// - arena can be created using Clay_CreateArenaWithCapacityAndMemory()
// - layoutDimensions are the initial bounding dimensions of the layout (i.e. the screen width and
// height for a full screen layout)
// - errorHandler is used by Clay to inform you if something has gone wrong in configuration or
// layout.
CLAY_DLL_EXPORT Clay_Context *Clay_Initialize(Clay_Arena arena, Clay_Dimensions layoutDimensions,
                                              Clay_ErrorHandler errorHandler);
// Returns the Context that clay is currently using. Used when using multiple instances of clay
// simultaneously.
CLAY_DLL_EXPORT Clay_Context *Clay_GetCurrentContext(void);
// Sets the context that clay will use to compute the layout.
// Used to restore a context saved from Clay_GetCurrentContext when using multiple instances of clay
// simultaneously.
CLAY_DLL_EXPORT void Clay_SetCurrentContext(Clay_Context *context);
// Updates the state of Clay's internal scroll data, updating scroll content positions if
// scrollDelta is non zero, and progressing momentum scrolling.
// - enableDragScrolling when set to true will enable mobile device like "touch drag" scroll of
// scroll containers, including momentum scrolling after the touch has ended.
...
CLAY_DLL_EXPORT void Clay_UpdateScrollContainers(bool enableDragScrolling, Clay_Vector2 scrollDelta,
                                                 float deltaTime);
...
// The returned Clay_ElementData contains a `found` bool that will be true if an element with the
// provided ID was found. This ID can be calculated either with CLAY_ID() for string literal IDs, or
// Clay_GetElementId for dynamic strings.
CLAY_DLL_EXPORT Clay_ElementData Clay_GetElementData(Clay_ElementId id);
// Returns true if the pointer position provided by Clay_SetPointerState is within the current
// element's bounding box. Works during element declaration, e.g. CLAY({ .backgroundColor =
// Clay_Hovered() ? BLUE : RED });
CLAY_DLL_EXPORT bool Clay_Hovered(void);
// Bind a callback that will be called when the pointer position provided by Clay_SetPointerState is
// within the current element's bounding box.
// - onHoverFunction is a function pointer to a user defined function.
// - userData is a pointer that will be transparently passed through when the onHoverFunction is
// called.
CLAY_DLL_EXPORT void Clay_OnHover(void (*onHoverFunction)(Clay_ElementId elementId,
                                                          Clay_PointerData pointerData,
                                                          void *userData),
                                  void *userData);
// An imperative function that returns true if the pointer position provided by Clay_SetPointerState
// is within the element with the provided ID's bounding box. This ID can be calculated either with
// CLAY_ID() for string literal IDs, or Clay_GetElementId for dynamic strings.
...
// The returned Clay_ScrollContainerData contains a `found` bool that will be true if a scroll
// element was found with the provided ID. An imperative function that returns true if the pointer
// position provided by Clay_SetPointerState is within the element with the provided ID's bounding
// box. This ID can be calculated either with CLAY_ID() for string literal IDs, or Clay_GetElementId
// for dynamic strings.
CLAY_DLL_EXPORT Clay_ScrollContainerData Clay_GetScrollContainerData(Clay_ElementId id);
// Binds a callback function that Clay will call to determine the dimensions of a given string
// slice.
// - measureTextFunction is a user provided function that adheres to the interface Clay_Dimensions
// (Clay_StringSlice text, Clay_TextElementConfig *config, void *userData);
// - userData is a pointer that will be transparently passed through when the measureTextFunction is
// called.
CLAY_DLL_EXPORT void Clay_SetMeasureTextFunction(
    Clay_Dimensions (*measureTextFunction)(Clay_StringSlice text, Clay_TextElementConfig *config,
                                           void *userData),
    void *userData);
// Experimental - Used in cases where Clay needs to integrate with a system that manages its own
// scrolling containers externally. Please reach out if you plan to use this function, as it may be
// subject to change.
CLAY_DLL_EXPORT void Clay_SetQueryScrollOffsetFunction(
    Clay_Vector2 (*queryScrollOffsetFunction)(uint32_t elementId, void *userData), void *userData);
// A bounds-checked "get" function for the Clay_RenderCommandArray returned from Clay_EndLayout().
CLAY_DLL_EXPORT Clay_RenderCommand *Clay_RenderCommandArray_Get(Clay_RenderCommandArray *array,
                                                                int32_t index);
...
// Enables and disables visibility culling. By default, Clay will not generate render commands for
// elements whose bounding box is entirely outside the screen.
...
// Returns the maximum number of measured "words" (whitespace seperated runs of characters) that
// Clay can store in its internal text measurement cache.
CLAY_DLL_EXPORT int32_t Clay_GetMaxMeasureTextCacheWordCount(void);
// Modifies the maximum number of measured "words" (whitespace seperated runs of characters) that
// Clay can store in its internal text measurement cache. This may require reallocating additional
// memory, and re-calling Clay_Initialize();
CLAY_DLL_EXPORT void Clay_SetMaxMeasureTextCacheWordCount(int32_t maxMeasureTextCacheWordCount);
// Resets Clay's internal text measurement cache. Useful if font mappings have changed or fonts have
// been reloaded.
...
CLAY_DLL_EXPORT Clay_ElementId Clay__HashStringWithOffset(Clay_String key, uint32_t offset,
                                                          uint32_t seed);
...
#define CLAY__ARRAY_DEFINE_FUNCTIONS(typeName, arrayName)                                          \
    ...
    arrayName arrayName##_Allocate_Arena(int32_t capacity, Clay_Arena *arena) {                    \
        return CLAY__INIT(arrayName){.capacity = capacity,                                         \
                                     .length = 0,                                                  \
    ...
    typeName *arrayName##_Get(arrayName *array, int32_t index) {                                   \
        return Clay__Array_RangeCheck(index, array->length) ? &array->internalArray[index]         \
                                                            : &typeName##_DEFAULT;                 \
    }                                                                                              \
                                                                                                   \
    ...
    typeName *arrayName##Slice_Get(arrayName##Slice *slice, int32_t index) {                       \
        return Clay__Array_RangeCheck(index, slice->length) ? &slice->internalArray[index]         \
                                                            : &typeName##_DEFAULT;                 \
    }                                                                                              \
                                                                                                   \
...
Clay_String Clay__WriteStringToCharBuffer(Clay__charArray *buffer, Clay_String string) {
    ...
    return CLAY__INIT(Clay_String){
        ...
        .chars = (const char *)(buffer->internalArray + buffer->length - string.length)};
}
...
#else
Clay_Dimensions (*Clay__MeasureText)(Clay_StringSlice text, Clay_TextElementConfig *config,
                                     void *userData);
...
Clay_TextElementConfig *Clay__StoreTextElementConfig(Clay_TextElementConfig config) {
    return Clay_GetCurrentContext()->booleanWarnings.maxElementsExceeded
               ...
               : Clay__TextElementConfigArray_Add(&Clay_GetCurrentContext()->textElementConfigs,
                                                  config);
}
Clay_AspectRatioElementConfig *
Clay__StoreAspectRatioElementConfig(Clay_AspectRatioElementConfig config) {
    return Clay_GetCurrentContext()->booleanWarnings.maxElementsExceeded
               ...
               : Clay__AspectRatioElementConfigArray_Add(
                     &Clay_GetCurrentContext()->aspectRatioElementConfigs, config);
}
Clay_ImageElementConfig *Clay__StoreImageElementConfig(Clay_ImageElementConfig config) {
    return Clay_GetCurrentContext()->booleanWarnings.maxElementsExceeded
               ? &Clay_ImageElementConfig_DEFAULT
               : Clay__ImageElementConfigArray_Add(&Clay_GetCurrentContext()->imageElementConfigs,
                                                   config);
}
...
Clay_CustomElementConfig *Clay__StoreCustomElementConfig(Clay_CustomElementConfig config) {
    return Clay_GetCurrentContext()->booleanWarnings.maxElementsExceeded
               ...
               : Clay__CustomElementConfigArray_Add(&Clay_GetCurrentContext()->customElementConfigs,
                                                    config);
}
Clay_ClipElementConfig *Clay__StoreClipElementConfig(Clay_ClipElementConfig config) {
    return Clay_GetCurrentContext()->booleanWarnings.maxElementsExceeded
               ? &Clay_ClipElementConfig_DEFAULT
               : Clay__ClipElementConfigArray_Add(&Clay_GetCurrentContext()->clipElementConfigs,
                                                  config);
}
Clay_BorderElementConfig *Clay__StoreBorderElementConfig(Clay_BorderElementConfig config) {
    return Clay_GetCurrentContext()->booleanWarnings.maxElementsExceeded
               ? &Clay_BorderElementConfig_DEFAULT
               : Clay__BorderElementConfigArray_Add(&Clay_GetCurrentContext()->borderElementConfigs,
                                                    config);
}
Clay_SharedElementConfig *Clay__StoreSharedElementConfig(Clay_SharedElementConfig config) {
    return Clay_GetCurrentContext()->booleanWarnings.maxElementsExceeded
               ? &Clay_SharedElementConfig_DEFAULT
               : Clay__SharedElementConfigArray_Add(&Clay_GetCurrentContext()->sharedElementConfigs,
                                                    config);
}
...
Clay_ElementConfig Clay__AttachElementConfig(Clay_ElementConfigUnion config,
    ...
    return *Clay__ElementConfigArray_Add(
        &context->elementConfigs, CLAY__INIT(Clay_ElementConfig){.type = type, .config = config});
}

Clay_ElementConfigUnion Clay__FindElementConfigWithType(Clay_LayoutElement *element,
                                                        Clay__ElementConfigType type) {
...
Clay_ElementId Clay__HashNumber(const uint32_t offset, const uint32_t seed) {
    ...
    return CLAY__INIT(Clay_ElementId){
        ...
        .offset = offset,
        .baseId = seed,
        .stringId = CLAY__STRING_DEFAULT}; // Reserve the hash result of zero as "null id"
}
...
Clay_ElementId Clay__HashString(Clay_String key, const uint32_t seed) {
    ...
    return CLAY__INIT(Clay_ElementId){.id = hash + 1,
                                      ...
                                      .baseId = hash + 1,
                                      .stringId =
                                          key}; // Reserve the hash result of zero as "null id"
}

Clay_ElementId Clay__HashStringWithOffset(Clay_String key, const uint32_t offset,
                                          const uint32_t seed) {
    uint32_t hash = 0;
    uint32_t base = seed;
...
Clay__MeasuredWord *Clay__AddMeasuredWord(Clay__MeasuredWord word,
    ...
    if (context->measuredWordsFreeList.length > 0) {
        uint32_t newItemIndex = Clay__int32_tArray_GetValue(
            &context->measuredWordsFreeList, (int)context->measuredWordsFreeList.length - 1);
...
Clay__MeasureTextCacheItem *Clay__MeasureTextCached(Clay_String *text,
        ...
        if (!context->booleanWarnings.textMeasurementFunctionNotSet) {
            ...
            context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
                ...
                .errorText = CLAY_STRING(
                    "Clay's internal MeasureText function is null. You may have forgotten to call "
                    "Clay_SetMeasureTextFunction(), or passed a NULL function pointer by mistake."),
    ...
    while (elementIndex != 0) {
        Clay__MeasureTextCacheItem *hashEntry =
            Clay__MeasureTextCacheItemArray_Get(&context->measureTextHashMapInternal, elementIndex);
        if (hashEntry->id == id) {
        ...
        if (context->generation - hashEntry->generation > 2) {
            ...
            while (nextWordIndex != -1) {
                Clay__MeasuredWord *measuredWord =
                    Clay__MeasuredWordArray_Get(&context->measuredWords, nextWordIndex);
    ...
    Clay__MeasureTextCacheItem newCacheItem = {
        .measuredWordsStartIndex = -1, .id = id, .generation = context->generation};
    Clay__MeasureTextCacheItem *measured = NULL;
    if (context->measureTextHashMapInternalFreeList.length > 0) {
        newItemIndex =
            Clay__int32_tArray_GetValue(&context->measureTextHashMapInternalFreeList,
                                        context->measureTextHashMapInternalFreeList.length - 1);
        context->measureTextHashMapInternalFreeList.length--;
        Clay__MeasureTextCacheItemArray_Set(&context->measureTextHashMapInternal, newItemIndex,
                                            newCacheItem);
        measured =
            Clay__MeasureTextCacheItemArray_Get(&context->measureTextHashMapInternal, newItemIndex);
    } else {
        if (context->measureTextHashMapInternal.length ==
            ...
            if (!context->booleanWarnings.maxTextMeasureCacheExceeded) {
                context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
                    ...
                    .errorText = CLAY_STRING(
                        "Clay ran out of capacity while attempting to measure text elements. Try "
                        "using Clay_SetMaxElementCount() with a higher value."),
                    .userData = context->errorHandler.userData});
        ...
        measured =
            Clay__MeasureTextCacheItemArray_Add(&context->measureTextHashMapInternal, newCacheItem);
    ...
    float spaceWidth =
        Clay__MeasureText(CLAY__INIT(Clay_StringSlice){.length = 1,
    ...
    while (end < text->length) {
        if (context->measuredWords.length == context->measuredWords.capacity - 1) {
            if (!context->booleanWarnings.maxTextMeasureCacheExceeded) {
                context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
                    ...
                    .errorText =
                        CLAY_STRING("Clay has run out of space in it's internal text measurement "
                                    "cache. Try using Clay_SetMaxMeasureTextCacheWordCount() "
                                    "(default 16384, with 1 unit storing 1 measured word)."),
                    .userData = context->errorHandler.userData});
                context->booleanWarnings.maxTextMeasureCacheExceeded = true;
            }
            return &Clay__MeasureTextCacheItem_DEFAULT;
        ...
        if (current == ' ' || current == '\n') {
            ...
            if (length > 0) {
                dimensions = Clay__MeasureText(
                    CLAY__INIT(Clay_StringSlice){
                        .length = length, .chars = &text->chars[start], .baseChars = text->chars},
                    config, context->measureTextUserData);
            }
            ...
            if (current == ' ') {
                ...
                previousWord =
                    Clay__AddMeasuredWord(CLAY__INIT(Clay__MeasuredWord){.startOffset = start,
                                                                         .length = length + 1,
    ...
    if (end - start > 0) {
        Clay_Dimensions dimensions = Clay__MeasureText(
            CLAY__INIT(Clay_StringSlice){
                .length = end - start, .chars = &text->chars[start], .baseChars = text->chars},
            config, context->measureTextUserData);
        Clay__AddMeasuredWord(
            CLAY__INIT(Clay__MeasuredWord){
                .startOffset = start, .length = end - start, .width = dimensions.width, .next = -1},
            previousWord);
        lineWidth += dimensions.width;
        measuredHeight = CLAY__MAX(measuredHeight, dimensions.height);
        measured->minWidth = CLAY__MAX(dimensions.width, measured->minWidth);
    ...
    if (elementIndexPrevious != 0) {
        Clay__MeasureTextCacheItemArray_Get(&context->measureTextHashMapInternal,
...
Clay_LayoutElementHashMapItem *Clay__AddHashMapItem(Clay_ElementId elementId,
                                                    Clay_LayoutElement *layoutElement) {
    Clay_Context *context = Clay_GetCurrentContext();
    ...
    while (hashItemIndex !=
           -1) { // Just replace collision, not a big deal - leave it up to the end user
        Clay_LayoutElementHashMapItem *hashItem = Clay__LayoutElementHashMapItemArray_Get(
        ...
        if (hashItem->elementId.id == elementId.id) { // Collision - resolve based on generation
            ...
            if (hashItem->generation <=
                context->generation) { // First collision - assume this is the "same" element
                hashItem->elementId =
                    elementId; // Make sure to copy this across. If the stringId reference has
                               // changed, we should update the hash item to use the new one.
                ...
                context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
                    ...
                    .errorText = CLAY_STRING("An element with this ID was already previously "
                                             "declared during this layout."),
                    .userData = context->errorHandler.userData});
    ...
    Clay_LayoutElementHashMapItem *hashItem =
        Clay__LayoutElementHashMapItemArray_Add(&context->layoutElementsHashMapInternal, item);
    hashItem->debugData = Clay__DebugElementDataArray_Add(
        &context->debugElementData, CLAY__INIT(Clay__DebugElementData) CLAY__DEFAULT_STRUCT);
    if (hashItemPrevious != -1) {
        Clay__LayoutElementHashMapItemArray_Get(&context->layoutElementsHashMapInternal,
                                                hashItemPrevious)
            ->nextIndex = (int32_t)context->layoutElementsHashMapInternal.length - 1;
    } else {
...
Clay_LayoutElementHashMapItem *Clay__GetHashMapItem(uint32_t id) {
    ...
    while (elementIndex != -1) {
        Clay_LayoutElementHashMapItem *hashEntry = Clay__LayoutElementHashMapItemArray_Get(
...
Clay_ElementId Clay__GenerateIdForAnonymousElement(Clay_LayoutElement *openLayoutElement) {
    ...
    Clay_LayoutElement *parentElement = Clay_LayoutElementArray_Get(
        ...
        Clay__int32_tArray_GetValue(&context->openLayoutElementStack,
                                    context->openLayoutElementStack.length - 2));
    uint32_t offset =
        parentElement->childrenOrTextContent.children.length + parentElement->floatingChildrenCount;
...
void Clay__UpdateAspectRatioBox(Clay_LayoutElement *layoutElement) {
    for (int32_t j = 0; j < layoutElement->elementConfigs.length; j++) {
        Clay_ElementConfig *config =
            Clay__ElementConfigArraySlice_Get(&layoutElement->elementConfigs, j);
        if (config->type == CLAY__ELEMENT_CONFIG_TYPE_ASPECT) {
            ...
            if (layoutElement->dimensions.width == 0 && layoutElement->dimensions.height != 0) {
                layoutElement->dimensions.width =
                    layoutElement->dimensions.height * aspectConfig->aspectRatio;
            } else if (layoutElement->dimensions.width != 0 &&
                       layoutElement->dimensions.height == 0) {
                layoutElement->dimensions.height =
                    layoutElement->dimensions.width * (1 / aspectConfig->aspectRatio);
            }
            break;
        }
    }
...
void Clay__CloseElement(void) {
    ...
    for (int32_t i = 0; i < openLayoutElement->elementConfigs.length; i++) {
        Clay_ElementConfig *config =
            Clay__ElementConfigArraySlice_Get(&openLayoutElement->elementConfigs, i);
    ...
    if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
        ...
        for (int32_t i = 0; i < openLayoutElement->childrenOrTextContent.children.length; i++) {
            ...
            Clay_LayoutElement *child =
                Clay_LayoutElementArray_Get(&context->layoutElements, childIndex);
            openLayoutElement->dimensions.width += child->dimensions.width;
            openLayoutElement->dimensions.height = CLAY__MAX(
                openLayoutElement->dimensions.height, child->dimensions.height + topBottomPadding);
            // Minimum size of child elements doesn't matter to clip containers as they can shrink
        ...
        float childGap =
            (float)(CLAY__MAX(openLayoutElement->childrenOrTextContent.children.length - 1, 0) *
        ...
        for (int32_t i = 0; i < openLayoutElement->childrenOrTextContent.children.length; i++) {
            ...
            openLayoutElement->dimensions.width = CLAY__MAX(
    ...
    if (layoutConfig->sizing.width.type != CLAY__SIZING_TYPE_PERCENT) {
        if (layoutConfig->sizing.width.size.minMax.max <=
            0) { // Set the max size if the user didn't specify, makes calculations easier
    ...
    bool elementIsFloating =
        Clay__ElementHasConfig(openLayoutElement, CLAY__ELEMENT_CONFIG_TYPE_FLOATING);

    // Close the currently open element
    int32_t closingElementIndex = Clay__int32_tArray_RemoveSwapback(
        &context->openLayoutElementStack, (int)context->openLayoutElementStack.length - 1);
...
void Clay__OpenElement(void) {
    ...
    if (context->openClipElementStack.length > 0) {
        Clay__int32_tArray_Set(
            &context->layoutElementClipElementIds, context->layoutElements.length - 1,
            Clay__int32_tArray_GetValue(&context->openClipElementStack,
...
void Clay__OpenTextElement(Clay_String text, Clay_TextElementConfig *textConfig) {
    ...
    Clay_ElementId elementId = Clay__HashNumber(
        parentElement->childrenOrTextContent.children.length + parentElement->floatingChildrenCount,
        parentElement->id);
    textElement->id = elementId.id;
    Clay__AddHashMapItem(elementId, textElement);
    Clay__StringArray_Add(&context->layoutElementIdStrings, elementId.stringId);
    Clay_Dimensions textDimensions = {.width = textMeasured->unwrappedDimensions.width,
                                      .height = textConfig->lineHeight > 0
                                                    ? (float)textConfig->lineHeight
                                                    : textMeasured->unwrappedDimensions.height};
    textElement->dimensions = textDimensions;
    textElement->minDimensions = CLAY__INIT(Clay_Dimensions){.width = textMeasured->minWidth,
                                                             .height = textDimensions.height};
    textElement->childrenOrTextContent.textElementData = Clay__TextElementDataArray_Add(
        &context->textElementData,
        CLAY__INIT(Clay__TextElementData){.text = text,
    ...
    textElement->elementConfigs = CLAY__INIT(Clay__ElementConfigArraySlice){
        ...
        .internalArray = Clay__ElementConfigArray_Add(
            ...
            CLAY__INIT(Clay_ElementConfig){.type = CLAY__ELEMENT_CONFIG_TYPE_TEXT,
...
void Clay__ConfigureOpenElementPtr(const Clay_ElementDeclaration *declaration) {
    ...
    if ((declaration->layout.sizing.width.type == CLAY__SIZING_TYPE_PERCENT &&
        ...
        context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
            ...
            .errorText = CLAY_STRING(
                "An element was configured with CLAY_SIZING_PERCENT, but the provided percentage "
                "value was over 1.0. Clay expects a value between 0 and 1, i.e. 20% is 0.2."),
            .userData = context->errorHandler.userData});
    }
    ...
    if (declaration->backgroundColor.a > 0) {
        sharedConfig = Clay__StoreSharedElementConfig(
            CLAY__INIT(Clay_SharedElementConfig){.backgroundColor = declaration->backgroundColor});
        Clay__AttachElementConfig(
            CLAY__INIT(Clay_ElementConfigUnion){.sharedElementConfig = sharedConfig},
            CLAY__ELEMENT_CONFIG_TYPE_SHARED);
    }
    if (!Clay__MemCmp((char *)(&declaration->cornerRadius), (char *)(&Clay__CornerRadius_DEFAULT),
                      sizeof(Clay_CornerRadius))) {
        if (sharedConfig) {
            sharedConfig->cornerRadius = declaration->cornerRadius;
        } else {
            sharedConfig = Clay__StoreSharedElementConfig(
                CLAY__INIT(Clay_SharedElementConfig){.cornerRadius = declaration->cornerRadius});
            Clay__AttachElementConfig(
                CLAY__INIT(Clay_ElementConfigUnion){.sharedElementConfig = sharedConfig},
                CLAY__ELEMENT_CONFIG_TYPE_SHARED);
        }
    ...
    if (declaration->userData != 0) {
        if (sharedConfig) {
            ...
            sharedConfig = Clay__StoreSharedElementConfig(
                CLAY__INIT(Clay_SharedElementConfig){.userData = declaration->userData});
            Clay__AttachElementConfig(
                CLAY__INIT(Clay_ElementConfigUnion){.sharedElementConfig = sharedConfig},
                CLAY__ELEMENT_CONFIG_TYPE_SHARED);
        }
    ...
    if (declaration->image.imageData) {
        Clay__AttachElementConfig(
            CLAY__INIT(Clay_ElementConfigUnion){
                .imageElementConfig = Clay__StoreImageElementConfig(declaration->image)},
        ...
        Clay__AttachElementConfig(
            CLAY__INIT(Clay_ElementConfigUnion){
                .aspectRatioElementConfig =
                    Clay__StoreAspectRatioElementConfig(declaration->aspectRatio)},
    ...
    if (declaration->floating.attachTo != CLAY_ATTACH_TO_NONE) {
        ...
        // This looks dodgy but because of the auto generated root element the depth of the tree
        // will always be at least 2 here
        ...
        if (hierarchicalParent) {
            ...
            if (declaration->floating.attachTo == CLAY_ATTACH_TO_PARENT) {
                ...
                if (parentItem == &Clay_LayoutElementHashMapItem_DEFAULT) {
                    context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
                        ...
                        .errorText = CLAY_STRING("A floating element was declared with a parentId, "
            ...
            int32_t currentElementIndex = Clay__int32_tArray_GetValue(
                &context->openLayoutElementStack, context->openLayoutElementStack.length - 1);
            Clay__int32_tArray_Set(&context->layoutElementClipElementIds, currentElementIndex,
                                   clipElementId);
    ...
    if (declaration->custom.customData) {
        Clay__AttachElementConfig(
            CLAY__INIT(Clay_ElementConfigUnion){
                .customElementConfig = Clay__StoreCustomElementConfig(declaration->custom)},
    ...
    if (declaration->clip.horizontal | declaration->clip.vertical) {
        Clay__AttachElementConfig(
            CLAY__INIT(Clay_ElementConfigUnion){
                .clipElementConfig = Clay__StoreClipElementConfig(declaration->clip)},
        ...
        if (!scrollOffset) {
            scrollOffset = Clay__ScrollContainerDataInternalArray_Add(
                ...
                CLAY__INIT(Clay__ScrollContainerDataInternal){.layoutElement = openLayoutElement,
                                                              .scrollOrigin = {-1, -1},
    ...
    if (!Clay__MemCmp((char *)(&declaration->border.width), (char *)(&Clay__BorderWidth_DEFAULT),
                      sizeof(Clay_BorderWidth))) {
        Clay__AttachElementConfig(
            CLAY__INIT(Clay_ElementConfigUnion){
                .borderElementConfig = Clay__StoreBorderElementConfig(declaration->border)},
...
void Clay__InitializeEphemeralMemory(Clay_Context *context) {
    ...
    context->aspectRatioElementConfigs =
        Clay__AspectRatioElementConfigArray_Allocate_Arena(maxElementCount, arena);
    ...
    context->treeNodeVisited.length =
        context->treeNodeVisited
            .capacity; // This array is accessed directly rather than behaving as a list
...
void Clay__InitializePersistentMemory(Clay_Context *context) {
    ...
    context->layoutElementsHashMapInternal =
        Clay__LayoutElementHashMapItemArray_Allocate_Arena(maxElementCount, arena);
    ...
    context->measuredWords =
        Clay__MeasuredWordArray_Allocate_Arena(maxMeasureTextCacheWordCount, arena);
...
void Clay__SizeContainersAlongAxis(bool xAxis) {
    ...
    for (int32_t rootIndex = 0; rootIndex < context->layoutElementTreeRoots.length; ++rootIndex) {
        ...
        Clay_LayoutElement *rootElement =
        ...
        if (Clay__ElementHasConfig(rootElement, CLAY__ELEMENT_CONFIG_TYPE_FLOATING)) {
            Clay_FloatingElementConfig *floatingElementConfig =
                Clay__FindElementConfigWithType(rootElement, CLAY__ELEMENT_CONFIG_TYPE_FLOATING)
        ...
        if (rootElement->layoutConfig->sizing.width.type != CLAY__SIZING_TYPE_PERCENT) {
            rootElement->dimensions.width =
                CLAY__MIN(CLAY__MAX(rootElement->dimensions.width,
        ...
        for (int32_t i = 0; i < bfsBuffer.length; ++i) {
            ...
            Clay_LayoutElement *parent =
                Clay_LayoutElementArray_Get(&context->layoutElements, parentIndex);
            ...
            float parentPadding = (float)(xAxis ? (parent->layoutConfig->padding.left +
            ...
            bool sizingAlongAxis =
                (xAxis && parentStyleConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) ||
            ...
            for (int32_t childOffset = 0;
                 childOffset < parent->childrenOrTextContent.children.length; childOffset++) {
                int32_t childElementIndex =
                ...
                Clay_SizingAxis childSizing = xAxis ? childElement->layoutConfig->sizing.width
                                                    : childElement->layoutConfig->sizing.height;
                float childSize =
                    xAxis ? childElement->dimensions.width : childElement->dimensions.height;
                ...
                if (childSizing.type != CLAY__SIZING_TYPE_PERCENT &&
                    ...
                    (!Clay__ElementHasConfig(childElement, CLAY__ELEMENT_CONFIG_TYPE_TEXT) ||
                     (Clay__FindElementConfigWithType(childElement, CLAY__ELEMENT_CONFIG_TYPE_TEXT)
                          .textElementConfig->wrapMode ==
                      CLAY_TEXT_WRAP_WORDS)) // todo too many loops
                    //                    && (xAxis || !Clay__ElementHasConfig(childElement,
                    //                    CLAY__ELEMENT_CONFIG_TYPE_ASPECT))
                ) {
                ...
                if (sizingAlongAxis) {
                    innerContentSize +=
                        (childSizing.type == CLAY__SIZING_TYPE_PERCENT ? 0 : childSize);
                    if (childSizing.type == CLAY__SIZING_TYPE_GROW) {
                        growContainerCount++;
                    }
                    if (childOffset > 0) {
                        innerContentSize +=
                            parentChildGap; // For children after index 0, the childAxisOffset is
                                            // the gap from the previous child
            ...
            if (sizingAlongAxis) {
                ...
                if (sizeToDistribute < 0) {
                    ...
                    while (sizeToDistribute < -CLAY__EPSILON &&
                        ...
                        for (int childIndex = 0; childIndex < resizableContainerBuffer.length;
                             childIndex++) {
                            Clay_LayoutElement *child = Clay_LayoutElementArray_Get(
                                &context->layoutElements,
                                Clay__int32_tArray_GetValue(&resizableContainerBuffer, childIndex));
                            float childSize =
                                xAxis ? child->dimensions.width : child->dimensions.height;
                        ...
                        widthToAdd = CLAY__MAX(widthToAdd,
                        ...
                        for (int childIndex = 0; childIndex < resizableContainerBuffer.length;
                            ...
                            float *childSize =
                                xAxis ? &child->dimensions.width : &child->dimensions.height;
                            float minSize =
                                xAxis ? child->minDimensions.width : child->minDimensions.height;
                            float previousWidth = *childSize;
                            if (Clay__FloatEqual(*childSize, largest)) {
                                *childSize += widthToAdd;
                                if (*childSize <= minSize) {
                                    *childSize = minSize;
                                    Clay__int32_tArray_RemoveSwapback(&resizableContainerBuffer,
                                                                      childIndex--);
                                }
                    ...
                    while (sizeToDistribute > CLAY__EPSILON &&
                        ...
                        widthToAdd = CLAY__MIN(widthToAdd,
                                               sizeToDistribute / resizableContainerBuffer.length);
                        ...
                        for (int childIndex = 0; childIndex < resizableContainerBuffer.length;
                            ...
                            float *childSize =
                                xAxis ? &child->dimensions.width : &child->dimensions.height;
                            float maxSize =
                                xAxis ? child->layoutConfig->sizing.width.size.minMax.max
                ...
                for (int32_t childOffset = 0; childOffset < resizableContainerBuffer.length;
                     childOffset++) {
                    ...
                    // If we're laying out the children of a scroll panel, grow containers expand to
                    // the size of the inner content, not the outer container
...
Clay_String Clay__IntToString(int32_t integer) {
    ...
    Clay_Context *context = Clay_GetCurrentContext();
    char *chars =
        (char *)(context->dynamicStringData.internalArray + context->dynamicStringData.length);
    int32_t length = 0;
    ...
    if (context->renderCommands.length < context->renderCommands.capacity - 1) {
        ...
        if (!context->booleanWarnings.maxRenderCommandsExceeded) {
            ...
            context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
                .errorType = CLAY_ERROR_TYPE_ELEMENTS_CAPACITY_EXCEEDED,
                .errorText =
                    CLAY_STRING("Clay ran out of capacity while attempting to create render "
                                "commands. This is usually caused by a large amount of wrapping "
                                "text elements while close to the max element capacity. Try using "
...
bool Clay__ElementIsOffscreen(Clay_BoundingBox *boundingBox) {
    ...
    return (boundingBox->x > (float)context->layoutDimensions.width) ||
           ...
           (boundingBox->x + boundingBox->width < 0) || (boundingBox->y + boundingBox->height < 0);
}
...
void Clay__CalculateFinalLayout(void) {
    ...
    // Wrap text
    for (int32_t textElementIndex = 0; textElementIndex < context->textElementData.length;
         ++textElementIndex) {
        Clay__TextElementData *textElementData =
        ...
        Clay_TextElementConfig *textConfig =
            Clay__FindElementConfigWithType(containerElement, CLAY__ELEMENT_CONFIG_TYPE_TEXT)
                .textElementConfig;
        Clay__MeasureTextCacheItem *measureTextCacheItem =
            Clay__MeasureTextCached(&textElementData->text, textConfig);
        float lineWidth = 0;
        float lineHeight = textConfig->lineHeight > 0 ? (float)textConfig->lineHeight
        ...
        if (!measureTextCacheItem->containsNewlines &&
            textElementData->preferredDimensions.width <= containerElement->dimensions.width) {
        ...
        while (wordIndex != -1) {
            ...
            if (lineLengthChars == 0 &&
                lineWidth + measuredWord->width > containerElement->dimensions.width) {
                Clay__WrappedTextLineArray_Add(
                    &context->wrappedTextLines,
                    CLAY__INIT(Clay__WrappedTextLine){
                        {measuredWord->width, lineHeight},
                        {.length = measuredWord->length,
                         .chars = &textElementData->text.chars[measuredWord->startOffset]}});
                textElementData->wrappedLines.length++;
            ...
            // measuredWord->length == 0 means a newline character
                ...
                bool finalCharIsSpace =
                    textElementData->text
                        .chars[CLAY__MAX(lineStartOffset + lineLengthChars - 1, 0)] == ' ';
                Clay__WrappedTextLineArray_Add(
                    &context->wrappedTextLines,
                    CLAY__INIT(Clay__WrappedTextLine){
                        {lineWidth + (finalCharIsSpace ? -spaceWidth : 0), lineHeight},
                        {.length = lineLengthChars + (finalCharIsSpace ? -1 : 0),
    ...
    for (int32_t i = 0; i < context->aspectRatioElementIndexes.length; ++i) {
        Clay_LayoutElement *aspectElement = Clay_LayoutElementArray_Get(
            &context->layoutElements,
            Clay__int32_tArray_GetValue(&context->aspectRatioElementIndexes, i));
        Clay_AspectRatioElementConfig *config =
            Clay__FindElementConfigWithType(aspectElement, CLAY__ELEMENT_CONFIG_TYPE_ASPECT)
                .aspectRatioElementConfig;
    ...
    for (int32_t i = 0; i < context->layoutElementTreeRoots.length; ++i) {
        Clay__LayoutElementTreeRoot *root =
            Clay__LayoutElementTreeRootArray_Get(&context->layoutElementTreeRoots, i);
        context->treeNodeVisited.internalArray[dfsBuffer.length] = false;
        Clay__LayoutElementTreeNodeArray_Add(
            &dfsBuffer, CLAY__INIT(Clay__LayoutElementTreeNode){
    ...
    while (dfsBuffer.length > 0) {
        ...
        if (!context->treeNodeVisited.internalArray[dfsBuffer.length - 1]) {
            ...
            // If the element has no children or is the container for a text element, don't bother
            // inspecting it
            ...
            // Add the children to the DFS buffer (needs to be pushed in reverse so that stack
        ...
        if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
            ...
            for (int32_t j = 0; j < currentElement->childrenOrTextContent.children.length; ++j) {
                ...
                float childHeightWithPadding =
                    CLAY__MAX(childElement->dimensions.height + layoutConfig->padding.top +
                                  layoutConfig->padding.bottom,
                              currentElement->dimensions.height);
                currentElement->dimensions.height = CLAY__MIN(
                    CLAY__MAX(childHeightWithPadding, layoutConfig->sizing.height.size.minMax.min),
                    layoutConfig->sizing.height.size.minMax.max);
            }
        } else if (layoutConfig->layoutDirection == CLAY_TOP_TO_BOTTOM) {
            ...
            contentHeight +=
                (float)(CLAY__MAX(currentElement->childrenOrTextContent.children.length - 1, 0) *
                        layoutConfig->childGap);
            currentElement->dimensions.height =
                CLAY__MIN(CLAY__MAX(contentHeight, layoutConfig->sizing.height.size.minMax.min),
                          layoutConfig->sizing.height.size.minMax.max);
        }
    }
    ...
    while (sortMax > 0) { // todo dumb bubble sort
        for (int32_t i = 0; i < sortMax; ++i) {
            ...
            Clay__LayoutElementTreeRoot next =
                *Clay__LayoutElementTreeRootArray_Get(&context->layoutElementTreeRoots, i + 1);
            if (next.zIndex < current.zIndex) {
                Clay__LayoutElementTreeRootArray_Set(&context->layoutElementTreeRoots, i, next);
                Clay__LayoutElementTreeRootArray_Set(&context->layoutElementTreeRoots, i + 1,
                                                     current);
            }
        }
        sortMax--;
    ...
    for (int32_t rootIndex = 0; rootIndex < context->layoutElementTreeRoots.length; ++rootIndex) {
        ...
        if (Clay__ElementHasConfig(rootElement, CLAY__ELEMENT_CONFIG_TYPE_FLOATING) &&
            ...
            switch (config->attachPoints.parent) { // I know I could merge the x and y switch
        ...
        if (root->clipElementId) {
            ...
            if (clipHashMapItem) {
                // Floating elements that are attached to scrolling contents won't be correctly
                // positioned if external scroll handling is enabled, fix here
                if (context->externalScrollHandlingEnabled) {
                    Clay_ClipElementConfig *clipConfig =
                        Clay__FindElementConfigWithType(clipHashMapItem->layoutElement,
                                                        CLAY__ELEMENT_CONFIG_TYPE_CLIP)
                            .clipElementConfig;
                ...
                Clay__AddRenderCommand(CLAY__INIT(Clay_RenderCommand){
                    ...
                    .userData = 0,
                    .id = Clay__HashNumber(rootElement->id,
                                           rootElement->childrenOrTextContent.children.length + 10)
                              .id, // TODO need a better strategy for managing derived ids
                    .zIndex = root->zIndex,
        ...
        Clay__LayoutElementTreeNodeArray_Add(
            ...
            CLAY__INIT(Clay__LayoutElementTreeNode){
                ...
                .nextChildOffset = {.x = (float)rootElement->layoutConfig->padding.left,
        ...
        while (dfsBuffer.length > 0) {
            ...
            if (!context->treeNodeVisited.internalArray[dfsBuffer.length - 1]) {
                ...
                Clay_BoundingBox currentElementBoundingBox = {
                    currentElementTreeNode->position.x, currentElementTreeNode->position.y,
                    currentElement->dimensions.width, currentElement->dimensions.height};
                ...
                if (Clay__ElementHasConfig(currentElement, CLAY__ELEMENT_CONFIG_TYPE_CLIP)) {
                    ...
                    // but I can't imagine a real UI with more than a few 10's of scroll containers
                    for (int32_t i = 0; i < context->scrollContainerDatas.length; i++) {
                        Clay__ScrollContainerDataInternal *mapping =
                ...
                while (sortMax > 0) { // todo dumb bubble sort
                    for (int32_t i = 0; i < sortMax; ++i) {
                        ...
                        Clay__ElementConfigType currentType =
                            Clay__ElementConfigArraySlice_Get(&currentElement->elementConfigs,
                                                              current)
                                ->type;
                        Clay__ElementConfigType nextType =
                            Clay__ElementConfigArraySlice_Get(&currentElement->elementConfigs, next)
                                ->type;
                ...
                for (int32_t elementConfigIndex = 0;
                     elementConfigIndex < currentElement->elementConfigs.length;
                     ++elementConfigIndex) {
                    Clay_ElementConfig *elementConfig = Clay__ElementConfigArraySlice_Get(
                        &currentElement->elementConfigs, sortedConfigIndexes[elementConfigIndex]);
                    ...
                    // Culling - Don't bother to generate render commands for rectangles entirely
                    // outside the screen - this won't stop their children from being rendered if
                    // they overflow
                    ...
                    case CLAY__ELEMENT_CONFIG_TYPE_TEXT: {
                        ...
                        float naturalLineHeight = currentElement->childrenOrTextContent
                                                      .textElementData->preferredDimensions.height;
                        float finalLineHeight = textElementConfig->lineHeight > 0
                        ...
                        for (int32_t lineIndex = 0;
                             lineIndex < currentElement->childrenOrTextContent.textElementData
                                             ->wrappedLines.length;
                             ++lineIndex) {
                            ...
                            float offset =
                                (currentElementBoundingBox.width - wrappedLine->dimensions.width);
                            ...
                            Clay__AddRenderCommand(CLAY__INIT(Clay_RenderCommand){
                                .boundingBox = {currentElementBoundingBox.x + offset,
                    ...
                    if (offscreen) {
                        // NOTE: You may be tempted to try an early return / continue if an element
                        // is off screen. Why bother calculating layout for its children, right?
                        // Unfortunately, a FLOATING_CONTAINER may be defined that attaches to a
                        // child or grandchild of this element, which is large enough to still be on
                        // screen, even if this element isn't. That depends on this element and it's
                        // children being laid out correctly (even if they are entirely off screen)
                    }
                }
                ...
                if (!Clay__ElementHasConfig(currentElementTreeNode->layoutElement,
                                            CLAY__ELEMENT_CONFIG_TYPE_TEXT)) {
                    Clay_Dimensions contentSize = {0, 0};
                    if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
                        for (int32_t i = 0;
                             i < currentElement->childrenOrTextContent.children.length; ++i) {
                            Clay_LayoutElement *childElement = Clay_LayoutElementArray_Get(
                                &context->layoutElements,
                                currentElement->childrenOrTextContent.children.elements[i]);
                            contentSize.width += childElement->dimensions.width;
                            contentSize.height =
                                CLAY__MAX(contentSize.height, childElement->dimensions.height);
                        }
                        contentSize.width +=
                            (float)(CLAY__MAX(
                                        currentElement->childrenOrTextContent.children.length - 1,
                                        0) *
                                    layoutConfig->childGap);
                        float extraSpace =
                        ...
                        for (int32_t i = 0;
                            ...
                            Clay_LayoutElement *childElement = Clay_LayoutElementArray_Get(
                                ...
                                currentElement->childrenOrTextContent.children.elements[i]);
                            contentSize.width =
                                CLAY__MAX(contentSize.width, childElement->dimensions.width);
                    ...
                    if (scrollContainerData) {
                        scrollContainerData->contentSize = CLAY__INIT(Clay_Dimensions){
                            contentSize.width +
                                (float)(layoutConfig->padding.left + layoutConfig->padding.right),
                            contentSize.height +
                                (float)(layoutConfig->padding.top + layoutConfig->padding.bottom)};
                ...
                Clay_ClipElementConfig *clipConfig =
                    Clay__FindElementConfigWithType(currentElement, CLAY__ELEMENT_CONFIG_TYPE_CLIP)
                        .clipElementConfig;
                if (clipConfig) {
                ...
                if (Clay__ElementHasConfig(currentElement, CLAY__ELEMENT_CONFIG_TYPE_BORDER)) {
                    ...
                    if (!Clay__ElementIsOffscreen(&currentElementBoundingBox)) {
                        Clay_SharedElementConfig *sharedConfig =
                            Clay__ElementHasConfig(currentElement, CLAY__ELEMENT_CONFIG_TYPE_SHARED)
                                ? Clay__FindElementConfigWithType(currentElement,
                                                                  CLAY__ELEMENT_CONFIG_TYPE_SHARED)
                                      .sharedElementConfig
                                : &Clay_SharedElementConfig_DEFAULT;
                        ...
                        Clay_RenderCommand renderCommand = {
                            .boundingBox = currentElementBoundingBox,
                            .renderData = {.border = {.color = borderConfig->color,
                                                      .cornerRadius = sharedConfig->cornerRadius,
                            ...
                            if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
                                for (int32_t i = 0;
                                    ...
                                    if (i > 0) {
                                        Clay__AddRenderCommand(CLAY__INIT(Clay_RenderCommand){
                                            .boundingBox =
                                                {currentElementBoundingBox.x + borderOffset.x +
                                                     scrollOffset.x,
                                                 currentElementBoundingBox.y + scrollOffset.y,
                                                 (float)borderConfig->width.betweenChildren,
                                                 currentElement->dimensions.height},
                                            .renderData = {.rectangle =
                                                               {
                                                                   .backgroundColor =
                                    ...
                                    borderOffset.x += (childElement->dimensions.width +
                                ...
                                for (int32_t i = 0;
                                    ...
                                    if (i > 0) {
                                        Clay__AddRenderCommand(CLAY__INIT(Clay_RenderCommand){
                                            .boundingBox =
                                                {currentElementBoundingBox.x + scrollOffset.x,
                                                 currentElementBoundingBox.y + borderOffset.y +
                                                     scrollOffset.y,
                                                 currentElement->dimensions.width,
                                                 (float)borderConfig->width.betweenChildren},
                                            .renderData = {.rectangle =
                                    ...
                                    borderOffset.y += (childElement->dimensions.height +
                                                       (float)layoutConfig->childGap);
                                }
                            }
                        }
                ...
                if (closeClipElement) {
                    Clay__AddRenderCommand(CLAY__INIT(Clay_RenderCommand){
                        .id = Clay__HashNumber(currentElement->id,
            ...
            if (!Clay__ElementHasConfig(currentElement, CLAY__ELEMENT_CONFIG_TYPE_TEXT)) {
                ...
                for (int32_t i = 0; i < currentElement->childrenOrTextContent.children.length;
                     ++i) {
                    Clay_LayoutElement *childElement = Clay_LayoutElementArray_Get(
                        &context->layoutElements,
                        currentElement->childrenOrTextContent.children.elements[i]);
                    ...
                    Clay_Vector2 childPosition = {
                        currentElementTreeNode->position.x +
                            currentElementTreeNode->nextChildOffset.x + scrollOffset.x,
                        currentElementTreeNode->position.y +
                            currentElementTreeNode->nextChildOffset.y + scrollOffset.y,
                    };

                    // DFS buffer elements need to be added in reverse because stack traversal
                    // happens backwards
                    ...
                    dfsBuffer.internalArray[newNodeIndex] = CLAY__INIT(Clay__LayoutElementTreeNode){
                        ...
                        .nextChildOffset = {.x = (float)childElement->layoutConfig->padding.left,
                                            .y = (float)childElement->layoutConfig->padding.top},
                    };
                    ...
                    if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
                        currentElementTreeNode->nextChildOffset.x +=
                            childElement->dimensions.width + (float)layoutConfig->childGap;
                    } else {
                        currentElementTreeNode->nextChildOffset.y +=
                            childElement->dimensions.height + (float)layoutConfig->childGap;
                    }
                }
            }
        }
...
Clay_TextElementConfig Clay__DebugView_TextNameConfig = {
    .textColor = {238, 226, 231, 255}, .fontSize = 16, .wrapMode = CLAY_TEXT_WRAP_NONE};
...
Clay__DebugGetElementConfigTypeLabel(Clay__ElementConfigType type) {
    switch (type) {
    case CLAY__ELEMENT_CONFIG_TYPE_SHARED:
        return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Shared"),
                                                                   {243, 134, 48, 255}};
    case CLAY__ELEMENT_CONFIG_TYPE_TEXT:
        return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Text"),
                                                                   {105, 210, 231, 255}};
    case CLAY__ELEMENT_CONFIG_TYPE_ASPECT:
        return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Aspect"),
                                                                   {101, 149, 194, 255}};
    case CLAY__ELEMENT_CONFIG_TYPE_IMAGE:
        return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Image"),
                                                                   {121, 189, 154, 255}};
    case CLAY__ELEMENT_CONFIG_TYPE_FLOATING:
        return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Floating"),
                                                                   {250, 105, 0, 255}};
    case CLAY__ELEMENT_CONFIG_TYPE_CLIP:
        return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Scroll"),
                                                                   {242, 196, 90, 255}};
    case CLAY__ELEMENT_CONFIG_TYPE_BORDER:
        return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Border"),
                                                                   {108, 91, 123, 255}};
    case CLAY__ELEMENT_CONFIG_TYPE_CUSTOM:
        return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Custom"),
                                                                   {11, 72, 107, 255}};
    default:
        break;
    }
    return CLAY__INIT(Clay__DebugElementConfigTypeLabelConfig){CLAY_STRING("Error"),
                                                               {0, 0, 0, 255}};
}
...
// Returns row count
Clay__RenderDebugLayoutData Clay__RenderDebugLayoutElementsList(int32_t initialRootsLength,
    ...
    for (int32_t rootIndex = 0; rootIndex < initialRootsLength; ++rootIndex) {
        ...
        context->treeNodeVisited.internalArray[0] = false;
        if (rootIndex > 0) {
            CLAY(CLAY_IDI("Clay__DebugView_EmptyRowOuter", rootIndex),
                 {.layout = {.sizing = {.width = CLAY_SIZING_GROW(0)},
                             .padding = {CLAY__DEBUGVIEW_INDENT_WIDTH / 2, 0, 0, 0}}}) {
                CLAY(CLAY_IDI("Clay__DebugView_EmptyRow", rootIndex),
                     {.layout = {.sizing = {.width = CLAY_SIZING_GROW(0),
        ...
        while (dfsBuffer.length > 0) {
            int32_t currentElementIndex =
                Clay__int32_tArray_GetValue(&dfsBuffer, (int)dfsBuffer.length - 1);
            Clay_LayoutElement *currentElement =
                Clay_LayoutElementArray_Get(&context->layoutElements, (int)currentElementIndex);
            if (context->treeNodeVisited.internalArray[dfsBuffer.length - 1]) {
                if (!Clay__ElementHasConfig(currentElement, CLAY__ELEMENT_CONFIG_TYPE_TEXT) &&
                    currentElement->childrenOrTextContent.children.length > 0) {
                    Clay__CloseElement();
            ...
            CLAY(CLAY_IDI("Clay__DebugView_ElementOuter", currentElement->id),
                ...
                if (!(Clay__ElementHasConfig(currentElement, CLAY__ELEMENT_CONFIG_TYPE_TEXT) ||
                    ...
                    CLAY(CLAY_IDI("Clay__DebugView_CollapseElement", currentElement->id),
                        ...
                        CLAY_TEXT((currentElementData && currentElementData->debugData->collapsed)
                                      ? CLAY_STRING("+")
                                      : CLAY_STRING("-"),
                                  CLAY_TEXT_CONFIG(
                                      {.textColor = CLAY__DEBUGVIEW_COLOR_4, .fontSize = 16}));
                    }
                    ...
                    if (currentElementData->debugData->collision) {
                        CLAY_AUTO_ID(
                            {.layout = {.padding = {8, 8, 2, 2}},
                             .border = {.color = {177, 147, 8, 255}, .width = {1, 1, 1, 1, 0}}}) {
                            CLAY_TEXT(CLAY_STRING("Duplicate ID"),
                                      CLAY_TEXT_CONFIG(
                                          {.textColor = CLAY__DEBUGVIEW_COLOR_3, .fontSize = 16}));
                        }
                    }
                    if (offscreen) {
                        CLAY_AUTO_ID({.layout = {.padding = {8, 8, 2, 2}},
                ...
                if (idString.length > 0) {
                    CLAY_TEXT(idString,
                              offscreen ? CLAY_TEXT_CONFIG({.textColor = CLAY__DEBUGVIEW_COLOR_3,
                                                            .fontSize = 16})
                ...
                for (int32_t elementConfigIndex = 0;
                     ...
                     ++elementConfigIndex) {
                    Clay_ElementConfig *elementConfig = Clay__ElementConfigArraySlice_Get(
                        &currentElement->elementConfigs, elementConfigIndex);
                    if (elementConfig->type == CLAY__ELEMENT_CONFIG_TYPE_SHARED) {
                        Clay_Color labelColor = {243, 134, 48, 90};
                        labelColor.a = 90;
                        Clay_Color backgroundColor =
                            elementConfig->config.sharedElementConfig->backgroundColor;
                        Clay_CornerRadius radius =
                            elementConfig->config.sharedElementConfig->cornerRadius;
                        if (backgroundColor.a > 0) {
                            CLAY_AUTO_ID(
                                ...
                                CLAY_TEXT(CLAY_STRING("Color"),
                                          CLAY_TEXT_CONFIG(
                                              {.textColor = offscreen ? CLAY__DEBUGVIEW_COLOR_3
                    ...
                    CLAY_AUTO_ID({.layout = {.padding = {8, 8, 2, 2}},
                        ...
                        CLAY_TEXT(
                            config.label,
                            CLAY_TEXT_CONFIG({.textColor = offscreen ? CLAY__DEBUGVIEW_COLOR_3
                                                                     : CLAY__DEBUGVIEW_COLOR_4,
                                              .fontSize = 16}));
                    }
                }
            ...
            if (Clay__ElementHasConfig(currentElement, CLAY__ELEMENT_CONFIG_TYPE_TEXT)) {
                ...
                Clay_TextElementConfig *rawTextConfig =
                    offscreen
                        ? CLAY_TEXT_CONFIG({.textColor = CLAY__DEBUGVIEW_COLOR_3, .fontSize = 16})
                        : &Clay__DebugView_TextNameConfig;
                CLAY_AUTO_ID(
                    {.layout = {.sizing = {.height = CLAY_SIZING_FIXED(CLAY__DEBUGVIEW_ROW_HEIGHT)},
                                .childAlignment = {.y = CLAY_ALIGN_Y_CENTER}}}) {
                    CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_FIXED(
                ...
                Clay__ConfigureOpenElement(
                    CLAY__INIT(Clay_ElementDeclaration){.layout = {.padding = {.left = 8}}});
                Clay__OpenElement();
                Clay__ConfigureOpenElement(CLAY__INIT(Clay_ElementDeclaration){
                    .layout = {.padding = {.left = CLAY__DEBUGVIEW_INDENT_WIDTH}},
            ...
            if (!(Clay__ElementHasConfig(currentElement, CLAY__ELEMENT_CONFIG_TYPE_TEXT) ||
                ...
                for (int32_t i = currentElement->childrenOrTextContent.children.length - 1; i >= 0;
                     --i) {
                    Clay__int32_tArray_Add(
                        &dfsBuffer, currentElement->childrenOrTextContent.children.elements[i]);
    ...
    if (highlightedElementId) {
        CLAY(CLAY_ID("Clay__DebugView_ElementHighlight"),
             {.layout = {.sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_GROW(0)}},
              .floating = {.parentId = highlightedElementId,
                           .zIndex = 32767,
                           .pointerCaptureMode = CLAY_POINTER_CAPTURE_MODE_PASSTHROUGH,
                           .attachTo = CLAY_ATTACH_TO_ELEMENT_WITH_ID}}) {
            CLAY(CLAY_ID("Clay__DebugView_ElementHighlightRectangle"),
                 {.layout = {.sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_GROW(0)}},
    ...
    CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_GROW(0)},
        ...
        CLAY_AUTO_ID({.layout = {.padding = {8, 8, 2, 2}},
            ...
            CLAY_TEXT(config.label,
                      CLAY_TEXT_CONFIG({.textColor = CLAY__DEBUGVIEW_COLOR_4, .fontSize = 16}));
        }
        CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_GROW(0)}}}) {
        }
        CLAY_TEXT(elementId, CLAY_TEXT_CONFIG({.textColor = CLAY__DEBUGVIEW_COLOR_3,
                                               .fontSize = 16,
...
void Clay__RenderDebugViewColor(Clay_Color color, Clay_TextElementConfig *textConfig) {
    CLAY_AUTO_ID({.layout = {.childAlignment = {.y = CLAY_ALIGN_Y_CENTER}}}) {
        ...
        CLAY_AUTO_ID({.layout = {.sizing = {CLAY_SIZING_FIXED(CLAY__DEBUGVIEW_ROW_HEIGHT - 8),
                                            CLAY_SIZING_FIXED(CLAY__DEBUGVIEW_ROW_HEIGHT - 8)}},
                      .backgroundColor = color,
                      .cornerRadius = CLAY_CORNER_RADIUS(4),
                      .border = {.color = CLAY__DEBUGVIEW_COLOR_4, .width = {1, 1, 1, 1, 0}}}) {
        }
    }
}
...
void HandleDebugViewCloseButtonInteraction(Clay_ElementId elementId, Clay_PointerData pointerInfo,
                                           void *userData) {
    Clay_Context *context = Clay_GetCurrentContext();
    (void)elementId;
    (void)pointerInfo;
...
void Clay__RenderDebugView(void) {
    Clay_Context *context = Clay_GetCurrentContext();
    Clay_ElementId closeButtonId =
        Clay__HashString(CLAY_STRING("Clay__DebugViewTopHeaderCloseButtonOuter"), 0);
    if (context->pointerInfo.state == CLAY_POINTER_DATA_PRESSED_THIS_FRAME) {
    ...
    Clay_TextElementConfig *infoTextConfig = CLAY_TEXT_CONFIG(
        {.textColor = CLAY__DEBUGVIEW_COLOR_4, .fontSize = 16, .wrapMode = CLAY_TEXT_WRAP_NONE});
    Clay_TextElementConfig *infoTitleConfig = CLAY_TEXT_CONFIG(
        {.textColor = CLAY__DEBUGVIEW_COLOR_3, .fontSize = 16, .wrapMode = CLAY_TEXT_WRAP_NONE});
    Clay_ElementId scrollId = Clay__HashString(CLAY_STRING("Clay__DebugViewOuterScrollPane"), 0);
    float scrollYOffset = 0;
    bool pointerInDebugView =
        context->pointerInfo.position.y < context->layoutDimensions.height - 300;
    for (int32_t i = 0; i < context->scrollContainerDatas.length; ++i) {
        ...
        if (scrollContainerData->elementId == scrollId.id) {
            if (!context->externalScrollHandlingEnabled) {
                ...
                pointerInDebugView =
                    context->pointerInfo.position.y + scrollContainerData->scrollPosition.y <
    ...
    CLAY(CLAY_ID("Clay__DebugView"),
         {.layout = {.sizing = {CLAY_SIZING_FIXED((float)Clay__debugViewWidth),
                                CLAY_SIZING_FIXED(context->layoutDimensions.height)},
        ...
        CLAY_AUTO_ID({.layout = {.sizing = {CLAY_SIZING_GROW(0),
            ...
            // Close button
            CLAY_AUTO_ID({
                .layout = {.sizing = {CLAY_SIZING_FIXED(CLAY__DEBUGVIEW_ROW_HEIGHT - 10),
        ...
        CLAY(scrollId,
             {.layout = {.sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_GROW(0)}},
              .clip = {
                  .horizontal = true, .vertical = true, .childOffset = Clay_GetScrollOffset()}}) {
            CLAY_AUTO_ID({.layout = {.sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_GROW(0)},
                                     .layoutDirection = CLAY_TOP_TO_BOTTOM},
                          .backgroundColor = ((initialElementsLength + initialRootsLength) & 1) == 0
                                                 ? CLAY__DEBUGVIEW_COLOR_2
                                                 : CLAY__DEBUGVIEW_COLOR_1}) {
                Clay_ElementId panelContentsId =
                    Clay__HashString(CLAY_STRING("Clay__DebugViewPaneOuter"), 0);
                // Element list
                ...
                CLAY_AUTO_ID({.layout = {.sizing = {.width = CLAY_SIZING_FIXED(contentWidth)},
                                         .layoutDirection = CLAY_TOP_TO_BOTTOM}}) {
                }
                for (int32_t i = 0; i < layoutData.rowCount; i++) {
                    Clay_Color rowColor =
                        (i & 1) == 0 ? CLAY__DEBUGVIEW_COLOR_2 : CLAY__DEBUGVIEW_COLOR_1;
        ...
        if (context->debugSelectedElementId != 0) {
            ...
            CLAY_AUTO_ID(
                ...
                CLAY_AUTO_ID(
                    ...
                    if (selectedItem->elementId.stringId.length != 0) {
                        ...
                        if (selectedItem->elementId.offset != 0) {
                            ...
                            CLAY_TEXT(Clay__IntToString(selectedItem->elementId.offset),
                                      infoTitleConfig);
                            CLAY_TEXT(CLAY_STRING(")"), infoTitleConfig);
                        }
                    }
                }
                Clay_Padding attributeConfigPadding = {CLAY__DEBUGVIEW_OUTER_PADDING,
                                                       CLAY__DEBUGVIEW_OUTER_PADDING, 8, 8};
                // Clay_LayoutConfig debug info
                CLAY_AUTO_ID({.layout = {.padding = attributeConfigPadding,
                                         .childGap = 8,
                                         .layoutDirection = CLAY_TOP_TO_BOTTOM}}) {
                    // .boundingBox
                    ...
                    CLAY_AUTO_ID({.layout = {.layoutDirection = CLAY_LEFT_TO_RIGHT}}) {
                        ...
                        CLAY_TEXT(Clay__IntToString(selectedItem->boundingBox.width),
                                  infoTextConfig);
                        CLAY_TEXT(CLAY_STRING(", height: "), infoTextConfig);
                        CLAY_TEXT(Clay__IntToString(selectedItem->boundingBox.height),
                                  infoTextConfig);
                        CLAY_TEXT(CLAY_STRING(" }"), infoTextConfig);
                    }
                    // .layoutDirection
                    ...
                    CLAY_TEXT(layoutConfig->layoutDirection == CLAY_TOP_TO_BOTTOM
                ...
                for (int32_t elementConfigIndex = 0;
                     elementConfigIndex < selectedItem->layoutElement->elementConfigs.length;
                     ++elementConfigIndex) {
                    Clay_ElementConfig *elementConfig = Clay__ElementConfigArraySlice_Get(
                        &selectedItem->layoutElement->elementConfigs, elementConfigIndex);
                    Clay__RenderDebugViewElementConfigHeader(selectedItem->elementId.stringId,
                                                             elementConfig->type);
                    switch (elementConfig->type) {
                    case CLAY__ELEMENT_CONFIG_TYPE_SHARED: {
                        Clay_SharedElementConfig *sharedConfig =
                        ...
                        CLAY_AUTO_ID({.layout = {.padding = attributeConfigPadding,
                            ...
                            Clay__RenderDebugViewColor(sharedConfig->backgroundColor,
                                                       infoTextConfig);
                            // .cornerRadius
                            CLAY_TEXT(CLAY_STRING("Corner Radius"), infoTitleConfig);
                            Clay__RenderDebugViewCornerRadius(sharedConfig->cornerRadius,
                                                              infoTextConfig);
                        }
                        break;
                    }
                    case CLAY__ELEMENT_CONFIG_TYPE_TEXT: {
                        ...
                        CLAY_AUTO_ID({.layout = {.padding = attributeConfigPadding,
                            ...
                            CLAY_TEXT(textConfig->lineHeight == 0
                                          ? CLAY_STRING("auto")
                    ...
                    case CLAY__ELEMENT_CONFIG_TYPE_ASPECT: {
                        ...
                        CLAY(CLAY_ID("Clay__DebugViewElementInfoAspectRatioBody"),
                            ...
                            CLAY(CLAY_ID("Clay__DebugViewElementInfoAspectRatio"), {}) {
                                CLAY_TEXT(Clay__IntToString(aspectRatioConfig->aspectRatio),
                                          infoTextConfig);
                                CLAY_TEXT(CLAY_STRING("."), infoTextConfig);
                                float frac = aspectRatioConfig->aspectRatio -
                    ...
                    case CLAY__ELEMENT_CONFIG_TYPE_IMAGE: {
                        ...
                        if (Clay__ElementHasConfig(selectedItem->layoutElement,
                                                   CLAY__ELEMENT_CONFIG_TYPE_ASPECT)) {
                            aspectConfig =
                                *Clay__FindElementConfigWithType(selectedItem->layoutElement,
                                                                 CLAY__ELEMENT_CONFIG_TYPE_ASPECT)
                                     .aspectRatioElementConfig;
                        }
                        CLAY(CLAY_ID("Clay__DebugViewElementInfoImageBody"),
                            ...
                            CLAY_AUTO_ID(
                                {.layout = {.sizing = {.width = CLAY_SIZING_GROW(64, 128),
                    ...
                    case CLAY__ELEMENT_CONFIG_TYPE_CLIP: {
                        ...
                        CLAY_AUTO_ID({.layout = {.padding = attributeConfigPadding,
                            ...
                            CLAY_TEXT(clipConfig->vertical ? CLAY_STRING("true")
                                                           : CLAY_STRING("false"),
                                      infoTextConfig);
                            // .horizontal
                            CLAY_TEXT(CLAY_STRING("Horizontal"), infoTitleConfig);
                            CLAY_TEXT(clipConfig->horizontal ? CLAY_STRING("true")
                                                             : CLAY_STRING("false"),
                                      infoTextConfig);
                        }
                        break;
                    ...
                    case CLAY__ELEMENT_CONFIG_TYPE_FLOATING: {
                        ...
                        CLAY_AUTO_ID({.layout = {.padding = attributeConfigPadding,
                            ...
                            CLAY_AUTO_ID({.layout = {.layoutDirection = CLAY_LEFT_TO_RIGHT}}) {
                                CLAY_TEXT(CLAY_STRING("{ x: "), infoTextConfig);
                                CLAY_TEXT(Clay__IntToString(floatingConfig->offset.x),
                                          infoTextConfig);
                                CLAY_TEXT(CLAY_STRING(", y: "), infoTextConfig);
                                CLAY_TEXT(Clay__IntToString(floatingConfig->offset.y),
                                          infoTextConfig);
                                CLAY_TEXT(CLAY_STRING(" }"), infoTextConfig);
                            }
                            // .expand
                            ...
                            CLAY_AUTO_ID({.layout = {.layoutDirection = CLAY_LEFT_TO_RIGHT}}) {
                                CLAY_TEXT(CLAY_STRING("{ width: "), infoTextConfig);
                                CLAY_TEXT(Clay__IntToString(floatingConfig->expand.width),
                                          infoTextConfig);
                                CLAY_TEXT(CLAY_STRING(", height: "), infoTextConfig);
                                CLAY_TEXT(Clay__IntToString(floatingConfig->expand.height),
                                          infoTextConfig);
                                CLAY_TEXT(CLAY_STRING(" }"), infoTextConfig);
                            }
                            // .zIndex
                            ...
                            CLAY_AUTO_ID({.layout = {.layoutDirection = CLAY_LEFT_TO_RIGHT}}) {
                                ...
                                } else if (floatingConfig->attachPoints.element ==
                                ...
                                } else if (floatingConfig->attachPoints.parent ==
                                           CLAY_ATTACH_POINT_LEFT_BOTTOM) {
                    ...
                    case CLAY__ELEMENT_CONFIG_TYPE_BORDER: {
                        ...
                        CLAY(CLAY_ID("Clay__DebugViewElementInfoBorderBody"),
                            ...
                            CLAY_AUTO_ID({.layout = {.layoutDirection = CLAY_LEFT_TO_RIGHT}}) {
                                CLAY_TEXT(CLAY_STRING("{ left: "), infoTextConfig);
                                CLAY_TEXT(Clay__IntToString(borderConfig->width.left),
                                          infoTextConfig);
                                CLAY_TEXT(CLAY_STRING(", right: "), infoTextConfig);
                                CLAY_TEXT(Clay__IntToString(borderConfig->width.right),
                                          infoTextConfig);
                                CLAY_TEXT(CLAY_STRING(", top: "), infoTextConfig);
                                CLAY_TEXT(Clay__IntToString(borderConfig->width.top),
                                          infoTextConfig);
                                CLAY_TEXT(CLAY_STRING(", bottom: "), infoTextConfig);
                                CLAY_TEXT(Clay__IntToString(borderConfig->width.bottom),
                                          infoTextConfig);
                                CLAY_TEXT(CLAY_STRING(" }"), infoTextConfig);
                            }
                            // .textColor
            ...
            CLAY(CLAY_ID("Clay__DebugViewWarningsScrollPane"),
                ...
                Clay_TextElementConfig *warningConfig =
                    CLAY_TEXT_CONFIG({.textColor = CLAY__DEBUGVIEW_COLOR_4,
                                      ...
                                      .wrapMode = CLAY_TEXT_WRAP_NONE});
                CLAY(
                    CLAY_ID("Clay__DebugViewWarningItemHeader"),
                    {.layout = {.sizing = {.height = CLAY_SIZING_FIXED(CLAY__DEBUGVIEW_ROW_HEIGHT)},
                                .padding = {CLAY__DEBUGVIEW_OUTER_PADDING,
                                            CLAY__DEBUGVIEW_OUTER_PADDING, 0, 0},
                                .childGap = 8,
...
Clay__WarningArray Clay__WarningArray_Allocate_Arena(int32_t capacity, Clay_Arena *arena) {
    ...
    if (nextAllocOffset + totalSizeBytes <= arena->capacity) {
        array.internalArray =
            (Clay__Warning *)((uintptr_t)arena->memory + (uintptr_t)nextAllocOffset);
        ...
        Clay__currentContext->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
            ...
            .errorText = CLAY_STRING(
                "Clay attempted to allocate memory in its arena, but ran out of capacity. Try "
                "increasing the capacity of the arena passed to Clay_Initialize()"),
...
bool Clay__Array_RangeCheck(int32_t index, int32_t length) {
    ...
    context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
        ...
        .errorText = CLAY_STRING("Clay attempted to make an out of bounds array access. This is an "
                                 "internal error and is likely a bug."),
        .userData = context->errorHandler.userData});
    return false;
}
...
uint32_t Clay_MinMemorySize(void) {
    ...
    // Reserve space in the arena for the context, important for calculating min memory size
    // correctly
...
void Clay_SetPointerState(Clay_Vector2 position, bool isPointerDown) {
    ...
    for (int32_t rootIndex = context->layoutElementTreeRoots.length - 1; rootIndex >= 0;
         --rootIndex) {
        dfsBuffer.length = 0;
        Clay__LayoutElementTreeRoot *root =
            Clay__LayoutElementTreeRootArray_Get(&context->layoutElementTreeRoots, rootIndex);
        ...
        while (dfsBuffer.length > 0) {
            ...
            Clay_LayoutElementHashMapItem *mapItem = Clay__GetHashMapItem(
                currentElement
                    ->id); // TODO think of a way around this, maybe the fact that it's essentially
                           // a binary tree limits the cost, but the worst case is not great
            ...
            if (mapItem) {
                ...
                if ((Clay__PointIsInsideRect(position, elementBox)) &&
                    (clipElementId == 0 ||
                     (Clay__PointIsInsideRect(position, clipItem->boundingBox)) ||
                     context->externalScrollHandlingEnabled)) {
                    if (mapItem->onHoverFunction) {
                        mapItem->onHoverFunction(mapItem->elementId, context->pointerInfo,
        ...
        Clay_LayoutElement *rootElement =
            Clay_LayoutElementArray_Get(&context->layoutElements, root->layoutElementIndex);
...
Clay_Context *Clay_Initialize(Clay_Arena arena, Clay_Dimensions layoutDimensions,
    ...
    *context = CLAY__INIT(Clay_Context){
        ...
        .maxMeasureTextCacheWordCount = oldContext ? oldContext->maxMeasureTextCacheWordCount
                                                   : Clay__defaultMaxMeasureTextWordCacheCount,
        .errorHandler = errorHandler.errorHandlerFunction
                            ? errorHandler
                            : CLAY__INIT(Clay_ErrorHandler){Clay__ErrorHandlerFunctionDefault, 0},
...
void Clay_UpdateScrollContainers(bool enableDragScrolling, Clay_Vector2 scrollDelta,
                                 float deltaTime) {
    Clay_Context *context = Clay_GetCurrentContext();
    bool isPointerActive =
        enableDragScrolling && (context->pointerInfo.state == CLAY_POINTER_DATA_PRESSED ||
                                context->pointerInfo.state == CLAY_POINTER_DATA_PRESSED_THIS_FRAME);
    ...
    for (int32_t i = 0; i < context->scrollContainerDatas.length; i++) {
        ...
        if (!scrollData->openThisFrame) {
            Clay__ScrollContainerDataInternalArray_RemoveSwapback(&context->scrollContainerDatas,
                                                                  i);
            continue;
        }
        ...
        if ((scrollData->scrollMomentum.x > -0.1f && scrollData->scrollMomentum.x < 0.1f) ||
            scrollOccurred) {
            scrollData->scrollMomentum.x = 0;
        }
        scrollData->scrollPosition.x =
            CLAY__MIN(CLAY__MAX(scrollData->scrollPosition.x,
        ...
        if ((scrollData->scrollMomentum.y > -0.1f && scrollData->scrollMomentum.y < 0.1f) ||
            scrollOccurred) {
            scrollData->scrollMomentum.y = 0;
        }
        scrollData->scrollPosition.y =
            CLAY__MIN(CLAY__MAX(scrollData->scrollPosition.y,
        ...
        for (int32_t j = 0; j < context->pointerOverIds.length;
             ++j) { // TODO n & m are small here but this being n*m gives me the creeps
    ...
    if (highestPriorityElementIndex > -1 && highestPriorityScrollData) {
        ...
        Clay_ClipElementConfig *clipConfig =
            Clay__FindElementConfigWithType(scrollElement, CLAY__ELEMENT_CONFIG_TYPE_CLIP)
                .clipElementConfig;
        ...
        if (isPointerActive) {
            ...
            if (!highestPriorityScrollData->pointerScrollActive) {
                ...
                if (scrollDeltaX > -0.1f && scrollDeltaX < 0.1f && scrollDeltaY > -0.1f &&
...
Clay_RenderCommandArray Clay_EndLayout(void) {
    ...
    if (context->booleanWarnings.maxElementsExceeded) {
        ...
        Clay__AddRenderCommand(CLAY__INIT(Clay_RenderCommand){
            ...
            .renderData = {.text = {.stringContents =
                                        CLAY__INIT(Clay_StringSlice){.length = message.length,
                                                                     .chars = message.chars,
    ...
    if (context->openLayoutElementStack.length > 1) {
        context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
            ...
            .errorText = CLAY_STRING(
                "There were still open layout elements when EndLayout was called. This results "
                "from an unequal number of calls to Clay__OpenElement and Clay__CloseElement."),
            .userData = context->errorHandler.userData});
    }
...
void Clay_OnHover(void (*onHoverFunction)(Clay_ElementId elementId, Clay_PointerData pointerInfo,
                                          void *userData),
                  void *userData) {
    Clay_Context *context = Clay_GetCurrentContext();
    if (context->booleanWarnings.maxElementsExceeded) {
...
bool Clay_PointerOver(
    Clay_ElementId elementId) { // TODO return priority for separating multiple results
    Clay_Context *context = Clay_GetCurrentContext();
    for (int32_t i = 0; i < context->pointerOverIds.length; ++i) {
...
Clay_ScrollContainerData Clay_GetScrollContainerData(Clay_ElementId id) {
    ...
    for (int32_t i = 0; i < context->scrollContainerDatas.length; ++i) {
        ...
        if (scrollContainerData->elementId == id.id) {
            Clay_ClipElementConfig *clipElementConfig =
                Clay__FindElementConfigWithType(scrollContainerData->layoutElement,
                                                CLAY__ELEMENT_CONFIG_TYPE_CLIP)
                    .clipElementConfig;
            if (!clipElementConfig) { // This can happen on the first frame before a scroll
            ...
            return CLAY__INIT(Clay_ScrollContainerData){
                ...
                .scrollContainerDimensions = {scrollContainerData->boundingBox.width,
...
Clay_ElementData Clay_GetElementData(Clay_ElementId id) {
    ...
    return CLAY__INIT(Clay_ElementData){.boundingBox = item->boundingBox, .found = true};
}
