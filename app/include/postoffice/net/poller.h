 ...
 * @param[in] poller The poller to destroy.
 ...
 * @param[in] events EPOLL event flags (e.g., EPOLLIN, EPOLLOUT).
 ...
 * @param[in] events New EPOLL event flags.
 * @return 0 on success, or -1 on error.
 * @note Thread-safe: Yes.
 */
 ...
 * @param[in] poller The poller instance.
 * @param[out] events Array of epoll_event to populate with ready events.
 * @param[in] max_events Maximum number of events to retrieve.
 * @param[in] timeout Maximum time to wait (milliseconds, -1 for infinite).
 * @return Number of events returned, or -1 on error.
 * @note Thread-safe: Yes (Multiple waiters allowed on epoll, but events shared).
 */
 ...
 *
 * @param[in] poller  Poller instance.
 * @param[out] events  Output array for events.
 * @param[in] max_events Capacity of events array.
 * @param[in] total_timeout_ms Total timeout budget in milliseconds (<=0 means immediate check).
 * @param[out] timed_out Optional out flag set to true when the timeout fully elapsed (no wake/events).
